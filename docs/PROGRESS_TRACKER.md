...
*Latest (2026-08-04, Landing page — LankaSeyla entry card) — ✅ **SHIPPED TO PRODUCTION + VERIFIED** (UI pipeline only; run [30958690466](https://github.com/Niroshana-SinharaRalalage/LankaConnect/actions/runs/30958690466) `success`, prod revision `lankaconnect-ui-prod--0000044`). Operator UAT'd staging ("Landing page looks good") then authorised production. **Prod verification**: home page HTTP 200 carrying `lankaseyla.lankaconnect.app` + `Clothing Store` + `Tradition Woven with Elegance` + `_blank`/`noopener`; browser-served logo is webp 443×110 with **ALPHA ✓ 74.1% transparent**; LankaEvents card intact (`>LankaEvents<`, `Event Planner`, exactly one `>Live<`, `/lanka-events` href). **Backend deliberately NOT deployed** — all 9 changed files are `web/` + one build script + four docs, zero `src/LankaConnect.*`, so no rebuild and **no EF migrations ran against the production DB**. ⚠️ **INCIDENT + PROCESS LESSON**: merging PR #137 to `main` auto-fired all three production workflows (`deploy-production.yml`, `deploy-ui-production.yml`, `deploy-production-with-approval.yml` all trigger on `push: branches: [main]`). Operator caught it; both live runs were cancelled within ~40s — backend died at step 5 "Build application" and UI at step 6 "Run type checking", with Azure Login / Docker push / Update Container App / **Run EF Core migrations** all showing `skipped`, so **production was never touched by the accidental runs**. **Correct process, now recorded in memory `feedback_main_merge_autotriggers_prod.md`: merging to `main` is a code sync only; the production push is dispatched from the `Production_<date>` release branch.** — earlier status for this item: **CODE COMPLETE + TESTS GREEN (12/12) + BUILD CLEAN**. **Operator request**: "Need a transperant version of the lankaseyla logo (instead of black backgryound). But do not make any other design changes to the logo. `C:\Niroshan\LankaConnect Marketplace\Logo Files` create and store it here and then use it." **2 artifacts, one script**: `scripts/generate-lanka-seyla-logo.js` now (1) writes **`LankaSeyla-transparent.png` (1600×887)** back beside the source JPEG in the brand Logo Files folder — same pixel dimensions, ONLY the alpha channel differs, no crop/scale/recolour/reflow — then (2) derives `web/public/lanka-seyla.png` (443×110, unchanged dims) from that master. **Alpha derivation**: the artwork is light text on a flat black field, so a pixel's max channel measures ink coverage directly; a max-channel histogram of the source showed **background 0–15 (1.35M px) and solid letterforms plateauing at ≥200 (47K px)** with only antialiased edges between, so alpha ramps linearly across 16→200 — glyphs fully opaque, field fully clear. **RGB copied through untouched rather than un-premultiplied**, so the brand gold stays exactly `#D7A959`; un-premultiplying would have divided by coverage and brightened it toward `#FFC869`. **Verified, not assumed**: alpha distribution 74.1% fully clear / 6.3% fully opaque / 19.6% partial (partial is high because at 443×110 most glyph pixels of a thin serif face ARE edge pixels), and compositing over magenta at 4× zoom showed clean cream/gold edges with **no dark fringe** — the failure mode this approach risks. Also composited over the card's gold-tinted dark background to confirm the gradient now shows through where the black rectangle used to sit. **No component or test changes** — asset dimensions are identical, so `EntryCard`, `page.tsx` and the natural-aspect test all stand. **TESTS**: 12/12 `landing-entry-cards.test.tsx`, 48/48 across `src/__tests__/pages/`, `tsc --noEmit` clean, `next build` compiled successfully. **Note**: `vitest` again aborted once with `[vitest-pool]: Timeout starting forks runner` (second occurrence — transient runner-startup flake, clean on immediate re-run; worth watching if it recurs). **Pending**: `deploy-ui-staging.yml` `workflow_dispatch` against `Production_05_09_2026` → operator re-UAT.*

*Earlier (2026-08-04, Landing page — LankaSeyla logo treatment fix-1) — **CODE COMPLETE + TESTS GREEN (12/12) + BUILD CLEAN; PUSHED TO `Production_05_09_2026`; STAGING REDEPLOY + OPERATOR RE-UAT PENDING**. **Operator UAT verbatim on the deployed 6A-adjacent landing change**: "why did you change the logo to fix to a squire shape? that is not needed. You can utilise logo as it is, just fix it to 110px hight. No need to change the length. utilizations doesn't need to exactly match with the existing LankaEvents button". **Correct call — the original design over-engineered the asset.** The first revision cropped the 4.03:1 wordmark into a stacked 110×110 "Lanka"/"Seyla" lockup purely to preserve structural parity with the LankaEvents square-icon slot; operator confirmed parity was never a requirement, and altering a brand's wordmark to satisfy a layout is the wrong trade. **3 changes**: (1) `scripts/generate-lanka-seyla-logo.js` simplified from a split/stack/pad/square pipeline to trim-then-scale-to-110px-height — wordmark used AS SUPPLIED, natural width follows (**443×110**, was 110×110), no cropping/splitting/reflowing; (2) `EntryCard` gains `logoWidth`/`logoHeight` (default 55×55, so LankaEvents is untouched) and `showName` — the square-icon assumption was baked into the component and had to come out; (3) `page.tsx` passes `logoWidth={221} logoHeight={55} showName={false}` for LankaSeyla. **`showName={false}` because the wordmark already spells the brand out** — printing "LankaSeyla" beside a logo reading "LankaSeyla" says it twice; the name still reaches assistive tech via the image `alt`, which is also what keeps `getByRole('link', {name:/LankaSeyla/i})` resolving. **Black background kept deliberately** — LankaSeyla's own site header sets the wordmark in a black box, so it reads as brand treatment, not artifact. **TESTS**: 12/12 GREEN in `landing-entry-cards.test.tsx` (+1 new: wordmark renders at natural aspect asserting width 221 ≠ height 55, so a future square-crop regression fails loudly; +1: brand name not duplicated beside the wordmark), 48/48 across `src/__tests__/pages/`, `tsc --noEmit` clean, `next build` compiled successfully. **Note**: one `vitest` run aborted with `[vitest-pool]: Timeout starting forks runner` — transient runner-startup flake, not a test failure; clean on immediate re-run. **Pending**: `deploy-ui-staging.yml` `workflow_dispatch` against `Production_05_09_2026` (that branch does NOT auto-deploy) → operator re-UAT.*

*Earlier (2026-08-04, Landing page — LankaSeyla sub-brand entry card) — **CODE COMPLETE + TESTS GREEN (11/11) + BUILD CLEAN; MERGED TO `Production_05_09_2026`; STAGING DEPLOY + OPERATOR UAT PENDING**. **No phase number claimed** — this is a UI-only landing-page change outside the Phase 6A events series, so no entry was reserved in `PHASE_6A_MASTER_INDEX.md`. **User request**: "In the LankaConnect landing page, I want to add another main entry button something similar to LankaEvents called LankaSeyla" — target confirmed as the external storefront `https://lankaseyla.lankaconnect.app/` (premium Sri Lankan clothing: Sarees & Jackets, Batik, Traditional, Casual & Party), opening in a new tab. **Design constraint discovered during brainstorming**: the supplied brand artwork (`LankaSeyla.jpeg`, 1600×887) is a **4.03:1 wordmark** once its black padding is trimmed (1268×315); letterboxed into the 110×110 square the card format expects it renders **14px tall** at the card's actual 55×55 display size, with the "by LankaConnect" sub-line illegible and ~70% of the square empty. Resolved by building the square asset as a **stacked lockup** — "Lanka" (white) over "Seyla" (gold), cut from the real letterforms with one shared scale factor so original type proportions survive (~17px per word at display size). Brand gold **sampled from the artwork**: `#D7A959` dominant (`#E3B565` highlights). **4 changes**: (1) NEW `scripts/generate-lanka-seyla-logo.js` — reproducible `sharp` pipeline (trim → split wordmark at x=700 → stack → pad → square → 110×110) writing `web/public/lanka-seyla.png`, committed so the asset is reproducible rather than an unexplained binary; (2) NEW `EntryCard.tsx` in `presentation/components/features/landing/` — extracted from the ~100 lines of inline JSX + four hand-written mouse handlers in `page.tsx` that each rebuilt a five-layer `boxShadow` from the brand colour; the arithmetic collapses into one `shadowFor(state, color)` helper so a new sub-brand is a five-line call instead of a 100-line copy-paste that drifts; (3) `page.tsx` middle band becomes `flex-col md:flex-row items-stretch` with both cards as `<EntryCard>` (LankaEvents pixel-identical, `ArrowUpRight` import dropped as now-unused, header comment corrected); (4) NEW `landing-entry-cards.test.tsx`. **LIVE dot deliberately omitted from the LankaSeyla card** — it signals live in-app data and on an external destination would assert liveness we cannot observe; `ArrowUpRight` carries the affordance alone. **Pre-existing a11y gap fixed in passing**: the card is a `Link` wrapping a `div` with no focus style — keyboard-reachable but invisible when focused; `EntryCard` adds a `focus-visible` ring, which repairs LankaEvents at the same time. **TDD ordering was the safety net**: the four LankaEvents assertions were written and run BEFORE the extraction (4 passed / 7 failed red), so the refactor of working UI had to prove it changed nothing rather than claim it — CLAUDE.md Section 3 treats such refactors as high-risk. **TESTS**: 11/11 GREEN in the new file, 47/47 across `src/__tests__/pages/` (no regression in `landing-page-metro-filtering`), `tsc --noEmit` clean, `next build` compiled successfully. **CI note discovered during deploy**: `deploy-ui-staging.yml` triggers on push to `develop` only — merging to `Production_05_09_2026` does NOT auto-deploy; staging requires a `workflow_dispatch` against that ref. Local `Production_05_09_2026` was also 117 commits behind origin and was fast-forwarded before the merge. **Spec**: [docs/superpowers/specs/2026-08-04-lankaseyla-landing-entry-design.md](./superpowers/specs/2026-08-04-lankaseyla-landing-entry-design.md). **Pending**: staging UI deploy → operator browser UAT (side-by-side layout at 320/768/1024px, the 100vh hero not clipping the bottom tagline, logo legibility at 55×55, new-tab behaviour).*

*Earlier (2026-06-01, Phase 6A.161 — Ticket Tier on the event-manage Attendees tab + CSV/Excel exports) — **CODE COMPLETE + TESTS GREEN + COMMITTED (`47ae66cf`); STAGING DEPLOY + API TESTS PENDING BRANCH COMPILE**. **User request**: "In the attendees tab we do not display the ticket tier. If available we should display them. Tier details should be added to CSV and EXCEL export files as well." **Architect-paired RCA classified as feature-missing (incomplete read-path), NOT a DB/Auth/UI-only/backend-defect**: the denormalized `ticket_tier_name` is ALREADY persisted per-attendee in the `registrations.attendees` JSONB (since migration `20260415203751_AddTicketTiers`, 2026-04-15) — no join to `ticket_tiers` needed — but it was dropped at the read path: the projection ([GetEventAttendeesQueryHandler.cs:129-134](src/LankaConnect.Application/Events/Queries/GetEventAttendees/GetEventAttendeesQueryHandler.cs#L129)) mapped only Name/AgeCategory/Gender, the shared `AttendeeDetailsDto` + `EventAttendeeDto` carried no tier field, and neither the UI table nor the backend-driven CSV/Excel exports rendered it. **5 read-path edits, NO migration**: (1) nullable `TicketTierId`+`TicketTierName` on shared `AttendeeDetailsDto`; (2) map both in the projection (no join) + tier-coverage log; (3) computed `EventAttendeeDto.TicketTierSummary` (uniform→single name, mixed→distinct-joined `VIP, General`, none→`—`; never null/blank; mirrors existing `AdditionalAttendees` idiom); (4) append one `Ticket Tier` column to BOTH `CsvExportService` + `ExcelExportService` (append-only so positional consumers don't break, blank in the TOTAL summary row, both writers in lockstep); (5) FE `AttendeeDto.ticketTierName` + `EventAttendeeDto.ticketTierSummary` types + a Ticket Tier column in `AttendeeManagementTab.tsx` (collapsed-row summary + per-attendee amber tier badge on expand; colSpan bumped 10→11 free / 13→14 paid). Nullable throughout — single-tier/free/legacy/Mode-B head-count registrations show `—`. **Display model PO-confirmed** (summary in collapsed row + per-attendee detail on expand). **TESTS**: 10/10 Infrastructure export tests GREEN (`Phase6A161TicketTierExportTests` — CSV+Excel parity, mixed-tier `VIP, General`, legacy null→`—`, free-event column present, append-position assertions) + 4 line-ending regression tests still GREEN; frontend `tsc --noEmit` clean. Dedicated Application unit tests (`Phase6A161TicketTierSummaryTests`, 6 cases) authored — production code compiles (`Application.dll` built) but the `Application.Tests` PROJECT can't execute yet because the sibling session's in-flight `SponsorTests.cs` (6A.162 brochure WIP) references `Sponsor.BrochureUrl`/`SetBrochure` that don't exist yet → 23 compile errors NOT mine. **Shared branch `feat/phase-6a-162-sponsor-two-images` per user direction (both Claude sessions on one branch); surgical path-staging — committed only my 11 files, never touched the sibling's `otherprompts2`/`SponsorTests.cs`.** **Build-env note**: concurrent `dotnet` builds in the same worktree collide on `obj/bin` (OOM/stack-overflow/sourcelink file-locks); resolved by building to a private `--artifacts-path`. **BLOCKED on**: branch can't deploy until the sibling's `SponsorTests.cs` compiles (CI builds the whole solution). Once green: trigger `deploy-staging.yml` + `deploy-ui-staging.yml` together → run staging API tests T1–T7 (tier fields on `/attendees`, mixed-tier summary, legacy null→`—`, CSV header last-col, Excel parity, UI smoke, container logs) → re-run `Application.Tests`. **Master TODO**: [docs/MASTER_TODO_PHASE_6A_161_ATTENDEE_TICKET_TIER_2026_06_01.md](./MASTER_TODO_PHASE_6A_161_ATTENDEE_TICKET_TIER_2026_06_01.md).*
