    ...
    public async Task<Result> Handle(UpdateSponsorConfigCommand request, CancellationToken cancellationToken)
    {
        ...
        using (LogContext.PushProperty("EventId", request.EventId))
        {
            ...
            try
            {
                ...
                // 4. Save + commit — Wave 8.5.g: direct-SaveChanges on LankaEventsDbContext
                await _dbContext.SaveChangesAsync(cancellationToken); // (was IUnitOfWork = 0 changes on AppDbContext)
