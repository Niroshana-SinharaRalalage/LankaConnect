...
|---|---|---|
| W1A — BuildingBlocks.Abstractions (NEW) | 2 | New csproj. Move `IUnitOfWork`, `ICommand`, `IQuery`, `IIdempotencyStore`, `IOutbox`, `ICurrentActor`, `IAuditLogger` here. `BuildingBlocks.Application` references it (impls stay where they are). Breaks the implicit Application→Domain coupling. |
