...
**Status**: ✅ **SHIPPED + STAGING-VERIFIED** (2026-05-01). Architect review iteration 1 applied (13 edits). 4 commits across 4 of 6 slices (7F-D.5 email template + 7F-D.6 cents-exact Stripe smoke deferred as post-ship follow-ups).

**Commits**:
- `430bbe23` (7F-D.1) — domain: `RegistrationAddition.CreateForHeadCountDelta` + `Registration.MergeHeadCountAddition` with mode-match invariant + currency-match + AdditionalAmount = newTotal − previousTotal tolerance + LeadAttendeeName preservation. 19 new domain tests covering factory invariants, B1+B1 / B2+B2 merges, cross-mode rejection, Mode A + Mode B delta rejection, max-attendees cap, status guards, tier-counts merge by TierId, null guards.
- `2f0371b5` (7F-D.2) — persistence: EF migration `Phase7FD_AddHeadCountDeltaToRegistrationAddition` adds `registration_mode` smallint NOT NULL DEFAULT 0 + `head_count_delta` jsonb NULL columns; CHECK constraint `ck_registration_additions_mode_xor` via raw SQL enforces Mode-A vs Mode-B mutual exclusion at storage layer (architect edit #6); jsonb deep-copy `ValueComparer` mirroring RegistrationConfiguration's pattern (memory 6A.129).
- `5bcf2a29` (7F-D.3) — application + API + webhook (merged per architect edit #12): `InitiateAddHeadCountCommand` + handler with full guards (Confirmed + Completed payment + IsHeadCountMode + single-pending-addition), `POST /api/events/registrations/{id}/add-headcount` (AllowAnonymous, mirrors /add-attendees pattern), `AdditionWebhookHandler` dispatches by `addition.IsModeBAddition` between `Registration.AddAttendees` (Mode A) and `Registration.MergeHeadCountAddition` (Mode B). New public `Event.CalculatePriceForHeadCount` wrapper for the application layer.
- `71145019` (7F-D.4) — frontend: new `AddHeadCountModal` with mode-aware spinners (B1 total / B2 adults+children / B3 males+females / B4 4-leaf cross), free-path immediate merge / paid-path Stripe redirect; `events/[id]/page.tsx` dispatches by `event.registrationMode`; 4 new RTL tests covering B2 / B4 spinner rendering + free-path submit + max-attendees cap. 15/15 events feature tests pass (no regressions in RsvpFormSection, ConvertRegistrationModeDialog, HeadCountRsvpForm.Phase7FC).

**Tests**: 19 new domain + 4 RTL = 23 new across the slice. Application suite **2513 / 6 skipped / 0 failed** (+19 over post-7F-B 2494). Architect floor was ≥24; actual 19 in domain alone.

**§6 staging end-to-end smoke (2026-05-01)** — strongest of the 7F suite:
- ✅ **Migration applied**: psycopg2 confirms `__EFMigrationsHistory` carries `20260501133949_Phase7FD_AddHeadCountDeltaToRegistrationAddition`; both `registration_mode` (smallint NOT NULL DEFAULT 0) and `head_count_delta` (jsonb) columns present; the architect-edit-#6 CHECK constraint `ck_registration_additions_mode_xor` is live with the exact predicate.
- ✅ **Endpoint registered**: differential probe — `POST /api/events/registrations/{id}/add-headcount-MISSING` returns HTTP 404 (route not found), `POST /api/events/registrations/{id}/add-headcount` returns HTTP 400 with `"Registration not found"` (route registered + AllowAnonymous + handler dispatched + registration lookup ran).
- ✅ **Full handler pipeline verified**: `POST /add-headcount` against a real B3 free-Mode-B registration returns HTTP 400 with the exact handler-coded message *"Registration must have a completed payment. Current status: NotRequired"* — proves: route → AllowAnonymous → command dispatch → registration loaded → PaymentStatus guard fired correctly. This is the strongest end-to-end evidence in the entire 7F suite (no auth bug to bypass since AllowAnonymous mirrors the Mode-A `/add-attendees` pattern).

**Cents-exact Stripe smoke deferred**: requires a paid Mode-B + tiered event with ChildPrice configured + a Confirmed registration, plus authenticated UI flow (the staging Auth issuer bug — JWTs anchored to 2026-04-25 — still blocks authenticated API smokes). The unit-level cents-exact contract is covered by the `Phase7E3cTierCountsPricingTests.Parity_ModeA_vs_ModeB_WithTierAge_BillsIdentically` parity test from 7F-C; the integration smoke can run once an organiser creates a fresh paid B-mode event in the UI.

**Backend deploys**: `25216879110` (7F-D.3) `conclusion=success`. Frontend deploy `25219755615` (7F-D.4) in flight at closeout.

**Slice 7F-D.5 (event-add-attendees-confirmation Mode-B variant) deferred** as a post-ship follow-up — the merge already works without an updated email template (the existing template fires on Mode-A and renders generic copy that's still readable for Mode B; the polished Mode-B-specific copy is a follow-up).

**Phase 7F end-to-end COMPLETE**: A↔B mode change (7F-B), tier × age matrix pricing on Mode B (7F-C), and paid Mode-B add-attendees with delta payment (7F-D) all live. Operators can now flip mode mid-event, charge per-tier-by-age, and let registrants expand their head-count after the initial RSVP.
