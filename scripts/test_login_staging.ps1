# Test staging login with detailed error handling
$body = @{
    email = "niroshhh@gmail.com"
    password = "12!@qwASzx"
    rememberMe = $true
    ipAddress = "string"
} | ConvertTo-Json

Write-Host "Attempting login to staging..."
Write-Host "URL: https://lankaconnect-api-staging.politebay-79d6e8a2.eastus2.azurecontainerapps.io/api/Auth/login"
Write-Host "Body: $body"

try {
    $response = Invoke-RestMethod -Uri 'https://lankaconnect-api-staging.politebay-79d6e8a2.eastus2.azurecontainerapps.io/api/Auth/login' `
        -Method POST `
        -ContentType 'application/json' `
        -Body $body `
        -ErrorAction Stop

    Write-Host "`nLogin successful!"
    Write-Host "Access Token: $($response.accessToken.Substring(0,30))..."
    $response.accessToken | Out-File -FilePath 'C:\Work\LankaConnect\.token.txt' -Encoding ascii -NoNewline
}
catch {
    Write-Host "`nLogin failed!" -ForegroundColor Red
    Write-Host "Status Code: $($_.Exception.Response.StatusCode.value__)"
    Write-Host "Error: $($_.Exception.Message)"

    if ($_.ErrorDetails.Message) {
        Write-Host "Details: $($_.ErrorDetails.Message)"
    }
}
