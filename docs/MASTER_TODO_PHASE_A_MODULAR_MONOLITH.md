...
| **Wave5** | Products carve-out (`Products/LankaEvents`) | ✅ **SHIPPED + CLOSED 2026-06-29** — Wave 5.0 SHIPPED (`916aab0b` skeleton); Wave 5.1 SHIPPED (`47e14ef9` + `59ed4483` Event-family Domain move); Wave 5.2 SHIPPED (~458 files Application carve-out across 10 sub-commits); Wave 5.3 SHIPPED + STAGING-VERIFIED 2026-06-29 (18-repo Infrastructure carve-out across `9be09e8a` + `bd33290a` + `a820df03` + `e43481cf` + `0047a6dd`; verification via Wave 9.a smoke 26 PASS / 0 FAIL); Wave 5.4 SHIPPED (`ae50fb27` interfaces + `c82ddce1` impls — completes 20-of-20 repo carve-out); Wave 5.5 SHIPPED (5.5.a `d64fb3a5` 8 ArchTest rules + 5.5.b `9e7a37ce` namespace comment cleanup + 5.5.c `bbbe6c12` Products README + MEMORY entry + 5.5.d blueprint §7.16 + Phase B note). Wave 6.5 carryover (DbContext extraction + EF Configs move + cross-schema FK policy + Outbox cutover) is scoped deferral per architect ruling 2026-06-29, not technical debt; tracked under "Wave 6 debt-tracking entries" above. See `src/Products/LankaEvents/README.md` for full status table. | 3 days actual (vs ~3 wk plan) |
