...
| **Wave7** | Frontend mirror (Turborepo + feature packages) + Wave 7.X.R (Roslyn analyzers for F17/F20 shape — see below) | ⏳ Pending | ~6 wk |
| **Wave 7.X.R** | Roslyn analyzers for F17-shape (`DbContext.Set<Dictionary<string,object>>("...")` shadow-table access) + F20-shape (`.IsRequired()` on nullable `DateTime?` / `Nullable<T>` domain property in EF Configuration) runtime-failure patterns. Currently caught by Wave 9 smoke; earlier build-gate valuable if pattern recurs. Architect ruling 2026-07-01 second-instance trigger: escalate for immediate analyzer authoring if EITHER pattern surfaces a THIRD confirmed bug (F17-shape now at 2 = deferrable; F20-shape at 1 = single-instance). Both patterns hard-to-express in NetArchTest.Rules (type-level), need Roslyn source-code inspection. | ⏳ PLANNED | Trigger-based execution: fire on 3rd instance of either shape. Bank in [[roslyn-analyzer-recurrence-trigger]] MEMORY. |
