...
| 2026-04-30 | 9.4 follow-up smoke | ✅ COMPLETE | API smoke discovered banquet preset bug (`ValidateForEvent` rejected zone-less banquet layouts). Fix shipped via commit `8b2b8d1b` (`!_zones.Any() && !_tables.Any()` instead of `!_zones.Any()`). Re-run smoke 4/4 PASS: T1 apply-template (200, atomic), T2 apply-preset replace (banquet 15 tables / 120 seats attaches; old layout becomes orphan invisible to by-event), T3 Slice 8 regression (8 presets / 409 stale-If-Match / 400 non-template-source for both old + new endpoints), T4 audit-table existence confirmed via deploy logs (runtime RAISE NOTICE counts not visible without DB access — design accepted). |
| 2026-04-30 | 9.5 | ✅ SHIPPED | Theater seat generation in canvas editor for empty zones. Backend: `BatchZone.RowCount` + `SeatsPerRow` optional fields; handler invokes `GenerateTheaterSeats` after add/update. Frontend: property panel renders Rows + "Seats per row" inputs when zone has 0 seats; `seatGenByZoneId` in draft state; payload forwarded via `composeBatchPayload`. Commit `6e11c1af` deployed (backend `25145376702` + frontend `25145376697` both `success`). Bug discovered via smoke: regen on populated zone returned 500 DB CHECK violation (`ck_seats_zone_xor_table`) because `Seat.VenueZoneId` is nullable (XOR with `VenueTableId`), making the EF FK optional → orphans seats by setting FK=null instead of deleting. Fix in commit `1b935ab6` (deploy `25146174322`): `GenerateTheaterSeats` now refuses regeneration on populated zones with precise message. Smoke verified: ADD new zone with `rowCount:3, seatsPerRow:5` → 204 + 15 seats; regen attempt on populated zone → 400 "Zone 'Main Floor' already has 200 seats. Delete the zone and re-add it to change the seat layout." 55/55 VenueLayoutTests pass; 2432 Application tests pass. **Deferred**: capacity input for tables (current default 8 works); curvature parameter; "regenerate seats" path with confirmation. |
