...
## 🎯 2026-05-25 (Phase 6A.153 — Organizer-controlled Registration Window) — ✅ BACKEND + UI STAGING-DEPLOYED + END-TO-END API VERIFIED (backend run 26431134833 SHA `a3bf7a4f`; UI run 26420331562 SHA `4f18eb3d`), awaiting operator browser UAT

Lets organizers publish events in advance with registration held closed until a date they choose. Click on the Register/RSVP button on a not-yet-open event opens a "Registration opens [LOCAL DATE]" modal instead of expanding the RSVP form. **Architect-locked design** across 11 decisions: two nullable UTC fields on `Event` (`RegistrationOpensAt`, `RegistrationClosesAt`) + `SetRegistrationWindow` mutator with invariants (opens < closes; closes <= start; opens < start; TBD events allowed any window; status-locked on Cancelled/Completed/Archived/Active); window guards in all 3 `Register*` domain methods via shared `GetRegistrationAvailability` reader (defense-in-depth); ExternalPaid dominates window in `RegisterWithAttendees` cascade (architect test #12 pinned). **EventDto** adds raw timestamps + new `RegistrationAvailability` string union (`open`/`not-yet-open`/`closed-by-organizer`/`closed-event-started`) — separate from existing `RegistrationModeStatus` (Phase 7E paid-B-mode gate) to preserve that field's semantics. **EF migration `Phase6A153_AddEventRegistrationWindow`** adds 2 nullable `timestamp with time zone` columns + 2 indexes; no data backfill (null = "always open" = backward-compatible default); `[Migration("...")]` attribute verified on Designer.cs per CLAUDE.md §8. **Frontend**: new `RegistrationWindowGate` component wraps the primary RsvpFormSection mount on `/events/[id]` — click-through modal for `not-yet-open` (matches user verbatim ask "show Registration/RSVP button but once anybody click it, we will display 'registration will open soon'"), inline microcopy for `closed-by-organizer` (user-confirmed 2026-05-25 — different UX because closure isn't a "come back later" state); organizer `EventCreationForm` + `EventEditForm` get 2 datetime pickers under "Registration Window (Optional)" with empty-string semantics ("Empty = open immediately" / "Empty = open until event start"). EventEditForm uses tri-state `UpdateRegistrationWindow` boolean so unrelated edits never accidentally clear a previously-set window. **Recovery flows deliberately bypass the gate** per existing convention (abandoned-checkout / payment-incomplete / refund-retry / cancellation-re-register). **Tests**: 24 mutator + 13 register-guard + 6 mapper + 7 RTL = 50 new tests GREEN; full app suite 2772/2778 (6 unrelated pre-existing FormResponseTests failures); TypeScript clean. **Staging API verification 2026-05-26**: 3 events created across not-yet-open / open / closed-by-organizer states; register returned `400 "Registration for this event opens at {iso-8601}"` / `200 success` / `400 "Registration for this event has closed"` respectively. **2 commits on `feat/phase-6a-153-registration-window`** (off main, not 148-bundle): `a3bf7a4f` backend + `4f18eb3d` frontend. **Note**: backend staging deploy initially overwritten by sibling W2.6a/b OpenTelemetry deploys (SHA `dd073bbf`); re-triggered run `26431134833` brought 6A.153 code back. **Operator UAT pending**: browse staging `/events/[id]` on the 3 created test events to confirm modal / form / closed-microcopy each render correctly.
