...
| **Wave5** | Products carve-out (`Products/LankaEvents`) | ✅ **SHIPPED + CLOSED 2026-06-30 (Wave 9.g closeout)** — All 16 commits landed (5.0/5.1/5.2/5.3/5.4/5.5.a-d). Verification: Wave 9 full suite (Wave 9.a-f) provides 254-test smoke baseline (119 PASS / 0 FAIL / 135 SKIP) across 25 controllers. Wave 5.3 finance-cluster repos (Sponsor, SponsorshipPackage, Donation, Collection, AddOn variants) NOW exercised via Wave 9.c + 9.e smokes. Wave 5.4 Analytics (EventAnalyticsRepository + EventViewRecordRepository) verified by dedicated AnalyticsController smoke -- 3/3 PASS. NO Wave 5 regressions surfaced; 19 platform findings cataloged in `docs/wave-9-findings.md` are all pre-existing (Wave 5 preserved existing behavior). Wave 6.5 carryover (DbContext + EF Configs + cross-schema FK + Outbox) is still scoped deferral per Wave 5.4 architect ruling. | 3 days code + 1 day Wave 9 verification |
...
| **Wave9** | **API Smoke Suite** — 7 sub-waves (a-g) + Wave 9.h "Eliminate avoidable SKIPs" | 🟢 **IN-PROGRESS (close stamp REVERTED 2026-06-30 evening)** — Wave 9.a `fa370be0` + 9.b `a9aa000e` + 9.c `5ce9e33f` + 9.d `4f0c6103` + 9.e `726c93dc` + 9.f `acefa6c9` + 9.g `0bb95120` shipped 254-test baseline (119 PASS / 0 FAIL / 135 SKIP). Founder ruled 2026-06-30 evening that ~110 SKIPs are lazy-skips (destructive without fixtures, missing Stripe test mode, missing multipart wrapper, etc.) and NOT valid per `[[feedback-no-skip-without-valid-reason]]`. **Wave 9.h NOW EXECUTING** -- 6 sub-slices per architect ruling: 9.h.1 fixture expansion, 9.h.4 multipart wrapper (foundation-first), 9.h.2 19-findings resolution, 9.h.3 destructive mutator coverage, 9.h.5 Stripe test mode + isolated admin, 9.h.6 real-comms isolation + LC_DISABLE_WEBHOOK_SIG_VALIDATION + OAuth issuance + Lc-Probe.psm1. Target post-9.h: ~220 PASS / ~5-7 SKIP. Wave 9 close re-stamps once 9.h ships + architect reviews. | 2 days actual (1 day 9.a-g + ~1 day 9.h) |
