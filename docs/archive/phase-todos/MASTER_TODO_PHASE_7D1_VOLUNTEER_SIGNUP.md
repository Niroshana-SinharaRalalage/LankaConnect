...
- [x] **C16a** — ✅ FIXED in G14 (migration `20260421190623_Phase7D1_FixVolunteerEmailTemplatePlaceholders`, commit `a81b16b7`, deploy `24741539754`). Targeted `REPLACE()` on both volunteer templates restored ToDictionary-compatible token names; staging render-diagnostic now emits zero unreplaced placeholders for `template-volunteer-commitment-cancellation`. Orthogonal defect `{{ItemName}}` remains in source signup-list templates (Phase 6A.102, pre-existing, affects both Items and Volunteers) — retracked below.
- [ ] **C16c** — Pre-existing Phase 6A.102 source-template defect: `template-signup-list-commitment-update` (and by clone-lineage, `template-volunteer-commitment-cancellation` text body) contains literal `{{ItemName}}`, `{{Notes}}`, `{{EventStartDate}}`, `{{EventStartTime}}`, `{{ManageCommitmentUrl}}` tokens that `SignupCommitmentEmailParams.ToDictionary()` does NOT emit. Logged via `[PLACEHOLDER-BUG]` diagnostic on every update-path send. Unrelated to Phase 7D.1 — owner-assigned when the Email Template Contract audit next runs.
...
- [x] **G14.** ✅ COMPLETE (commit `a81b16b7`, deploy run `24741539754`). Migration `20260421190623_Phase7D1_FixVolunteerEmailTemplatePlaceholders` — targeted `REPLACE()` SQL over both volunteer templates (`html_template`/`text_template`/`subject_template`) restores ToDictionary-compatible Handlebars token names (`{{VolunteerListUrl}}`→`{{SignupListUrl}}`, `{{#HasVolunteerLists}}`/`{{/HasVolunteerLists}}`/`{{HasVolunteerLists}}`→`{{#HasSignUpLists}}`/`{{/HasSignUpLists}}`/`{{HasSignUpLists}}`, matching pair for `VolunteerForms`/`SignupForms`). Row-count assertion per MEMORY Phase 6A.117 fails migration if 0 rows matched. **Evidence**: CI "Run EF Migrations" step ✓ (proves WHERE-clause matched → UPDATE ran → affected ≥ 1 on both templates). Staging cancel-flow smoke (event `d543629f`, user `niroshhh`, `POST /commit {quantity:0}`): `[PLACEHOLDER-BUG]` diagnostic emitted **zero** warnings on `template-volunteer-commitment-cancellation` render (contrast: same log run showed 5 unreplaced `{{ItemName}}`/`{{Notes}}`/`{{EventStartDate}}`/`{{EventStartTime}}`/`{{ManageCommitmentUrl}}` on pre-existing `template-signup-list-commitment-update` — Phase 6A.102 source-template bug, out-of-scope). Azure ACS send succeeded in 10803ms, Operation ID `89dd53f0-0e7d-4a55-bb0c-553329561cca`. `{{ItemName}}` in volunteer text template is a PRE-EXISTING Phase 6A.102 source-template defect (affects both Items and Volunteers) — deliberately out-of-scope; tracked as C16a.
