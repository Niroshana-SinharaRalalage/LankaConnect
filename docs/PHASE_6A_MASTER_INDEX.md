...
| 6A.146 | Public Form Responses with PII Redaction: organizer toggle on signup form create/edit page (default OFF) to allow event visitors to view all form responses; new `AllowAttendeesToViewResponses` boolean on `EventForm` aggregate with EF migration adding `boolean NOT NULL DEFAULT false` column; new `[AllowAnonymous] GET .../forms/{formId}/responses/public` endpoint with defense-in-depth handler (returns 404 if flag off or status Draft/Archived to avoid leaking toggle state); separate `PublicFormResponseDto` (compile-time PII guarantee — no `RespondentName`/`RespondentEmail`/`RespondentUserId` fields physically present) with ordinal `RespondentLabel` ("Respondent N" by `SubmittedAt ASC`) and date-only `SubmittedOn`; new `PublicFormResponsesSection` on event detail page mirroring `SignUpManagementSection` collapsible-card pattern; question-content PII risk accepted as organizer responsibility, called out in UI copy | 🔧 In Progress | Architect-approved plan; user-locked decisions: ordinal labels / Active+Closed status gate / date-only `SubmittedOn` / new branch deferred to current branch per user choice | 2026-05-15 |
