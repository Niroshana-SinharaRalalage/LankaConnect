...
{
    /// <inheritdoc />
    public partial class Phase6A123_FixSignUpCommitmentsSchema : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            // Phase 6A.123: CRITICAL FIX - quantity column in sign_up_commitments is NOT NULL with no default.
            // Phase 6A.121 added builder.Ignore(c => c.Quantity) to EF Core config, so EF Core no longer
            // generates INSERT statements that include the quantity column.
            // Without a DEFAULT, every INSERT to sign_up_commitments fails with PostgreSQL NOT NULL violation.
            // Fix: Set DEFAULT 0 so existing rows and new inserts without quantity work correctly.
