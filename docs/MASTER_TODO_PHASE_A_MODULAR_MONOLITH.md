...
| Item | Trigger | Notes |
|---|---|---|
| **Roslyn analyzers for F17-shape + F20-shape** | 3rd instance of either pattern (F17-shape currently at 2 = deferrable; F20-shape at 1 = single-instance) | F17 shape = `DbContext.Set<Dictionary<string,object>>("...")` shadow-table access after junction CLR-typed. F20 shape = EF `.IsRequired()` on `Nullable<T>` domain property in Configuration. Both hard-to-express in NetArchTest.Rules (need Roslyn source-code inspection). Currently caught by Wave 9 smoke; earlier build-gate valuable only if pattern recurs. Per architect ruling 2026-07-01: two data points = deferrable pattern; three = confirmed recurring class = analyzer justified. Escalate to architect on 3rd instance for immediate analyzer authoring. See `[[roslyn-analyzer-recurrence-trigger]]` memory rule. |
