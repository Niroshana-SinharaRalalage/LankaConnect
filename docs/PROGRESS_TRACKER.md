...
**Next**: S8.8 split into **S8.8a backend** (wire `_metrics.LayoutCanvasEditorSaved(layoutId, changesCount)` in [BatchUpdateLayoutCommandHandler.cs](../src/LankaConnect.Application/Events/Commands/BatchUpdateLayout/BatchUpdateLayoutCommandHandler.cs) — interface + emitter exist; only the call site is missing) + **S8.8b frontend** (compose `BatchLayoutPayload` from draft, add Save button to modal footer, wire `useBatchUpdateVenueLayout` + `recordCanvasEditorSaved`, 409 reload UX). Then S8.9 — save-as-personal-template + warn-before-close.
