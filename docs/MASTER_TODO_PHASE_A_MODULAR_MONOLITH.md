...
| **Wave9** | **API Smoke Suite** — per-controller staging smoke covering 25 of 42 controllers + 3 cross-controller scenarios + deploy-staging.yml CI hook | ✅ **SHIPPED + CLOSED 2026-06-30** — Wave 9.a `fa370be0` (Foundation + Events) + 9.b `a9aa000e` (Auth + Identity, 40 endpoints) + 9.c `5ce9e33f` (Venue + ticketing, 44 endpoints) + 9.d `4f0c6103` (Communications, 47 endpoints) + 9.e `726c93dc` (Finance + business, 53 endpoints; CRITICAL Wave 5.3 verification) + 9.f `acefa6c9` (Analytics + LongTail + 3 scenarios + CI hook) + 9.g (this commit; closeout + Wave 5 close). Smoke baseline: 254 tests, 119 PASS / 0 FAIL / 135 SKIP. 19 platform findings cataloged in `docs/wave-9-findings.md` (hardening candidates for Wave 6 or dedicated W6.X). CI integration: `.github/workflows/deploy-staging.yml` runs Run-Wave9.ps1 on every deploy; failures block deploy. | 1 day actual (vs ~3 wk plan; massive smoke discipline acceleration) |
