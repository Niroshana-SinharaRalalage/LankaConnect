...
*Latest (2026-05-25, Phase 6A.153 — **Organizer-controlled Registration Window**) — **COMMITTED + STAGING-DEPLOYED + END-TO-END API VERIFIED (backend run 26431134833 SHA `a3bf7a4f`, UI run 26420331562 SHA `4f18eb3d`)**. Lets organizers publish events in advance with registration held closed until a date they choose. Register/RSVP button still renders; click on a not-yet-open event opens a "Registration opens [LOCAL DATE]" modal instead of the form. **Architect-locked design across 11 decisions**: two nullable UTC fields (`RegistrationOpensAt`, `RegistrationClosesAt`) on `Event`; new `SetRegistrationWindow(opensAt, closesAt)` mutator with invariants (opens < closes; closes <= start; opens < start; TBD events allowed any window; locked in Cancelled/Completed/Archived/Active); window guards in all 3 `Register*` domain methods via shared `GetRegistrationAvailability` reader (defense-in-depth, single source of truth); ExternalPaid dominates window in cascade (architect test #12 pinned). **DTO + mapper**: raw `RegistrationOpensAt`/`RegistrationClosesAt` timestamps + computed `RegistrationAvailability` string union (`"open" | "not-yet-open" | "closed-by-organizer" | "closed-event-started"`) — separate from `RegistrationModeStatus` (Phase 7E paid-B-mode gate) to preserve the latter's semantics. **EF migration `Phase6A153_AddEventRegistrationWindow`**: two nullable `timestamp with time zone` columns + two indexes; `[Migration("...")]` attribute verified on Designer.cs; no data backfill (null = "always open" = backward-compatible default). **Frontend**: new `RegistrationWindowGate` component wraps the primary RsvpFormSection mount site on `/events/[id]` — click-through modal for `not-yet-open` (matches user verbatim ask), inline microcopy for `closed-by-organizer` (user-confirmed 2026-05-25); organizer EventCreationForm + EventEditForm get 2 datetime pickers under "Registration Window (Optional)" with empty-string semantics documented. EventEditForm uses tri-state `UpdateRegistrationWindow` boolean so unrelated edits don't accidentally clear a previously-set window. **Recovery flows deliberately bypass the gate** per existing RsvpFormSection mount-site convention (abandoned-checkout / payment-incomplete / refund-retry / cancellation-re-register paths). **Tests**: 24 `Event_SetRegistrationWindow_Tests` (mutator + boundary semantics), 13 `Event_RegisterRespectsRegistrationWindow_Tests` (window guards × 3 Register* methods + ExternalPaid-dominates-window cascade pin), 6 `EventMappingProfileRegistrationAvailabilityTests` (4 states + raw-timestamp pass-through), 7 `RegistrationWindowGate.test.tsx` RTL (open/undefined/closed-event-started → render children; not-yet-open → CTA + modal + formatted-time copy; closed-by-organizer → inline microcopy). Full app test suite 2772/2778 GREEN (6 unrelated pre-existing failures in FormResponseTests). **Staging API verification 2026-05-26 03:52 UTC**: 3 test events created and registered against — (a) `2c7ca1da` opens=+7d / closes=+25d → register returns 400 `"Registration for this event opens at 2026-06-01T21:51:42.8609450Z"` ✅; (b) `18f6fbcf` opens=-1d / closes=+25d → register returns 200 success ✅; (c) `b93a8e7e` opens=-5d / closes=-10s → register returns 400 `"Registration for this event has closed"` ✅. **Note**: backend deploy initially overwritten by sibling W2.6a/b OpenTelemetry deploys (SHA `dd073bbf`); re-deployed `a3bf7a4f` on top so staging is currently on 6A.153 code. **Branched off `main`** (post-6A.152 sync); does NOT ride the 148-bundle branch — clean isolated PR. **Operator UAT pending**: browser the staging /events/[id] page on the 3 test events above — click Register on `2c7ca1da` should show modal with formatted local time; `18f6fbcf` should show normal form; `b93a8e7e` should show inline "Registration is closed" microcopy.*
