...
| W9.a | Foundation modules (Lc-Http/Auth/Assertion/Report/EventFixtures/CommonFixtures) + Smoke-EventsController.ps1 (13 sub-sections) + Run-Wave9a.ps1 orchestrator + W5.3 STAGING-VERIFIED flip — gated W5.3 + Wave 5 resumption | Wave 9.a | Wave 9 | Phase A | V5 | SHIPPED `fa370be0` (Foundation+Smoke) + `245e4982` (modules+Pester 77/77) — STAGING-VERIFIED smoke 2026-06-29 (26 PASS / 0 FAIL / 8 SKIP) | Planning Agent + Architect pairing |
| W9.b-g | Per-controller smoke for remaining 41 controllers (~330 endpoints total). Sub-slices: Auth+Identity / Communications / Finance+Business / Long-tail / Scenarios / CI hook | Wave 9.b through g | Wave 9 | Phase A | V5 | PLANNED | Planning Agent |
| W5.4 | Products carve-out resumption — scope TBD per architect consult (likely AppDbContext partition / module DbContext extraction / cross-schema FK resolution). Uses Wave 9.a smoke for per-slice verification (no UI checkpoints). | Wave 5.4 | Wave 5 | Phase A | V2 + V3 | PLANNED — AWAITING ARCHITECT CONSULT | Planning Agent + Architect pairing |
...
| SPRINT-D10.1 | Restore full-solution build in `.github/workflows/deploy-staging.yml` step 5 (revert to `dotnet build -c Release --no-restore`). Prerequisite: ~72 test-project compile errors cleared during Day 7-8 fix-forward. Per Consult #18 (2026-07-10) transitional workflow-scope-narrowing during Sprint Day 6→10. | Sprint Day 10 slot A | Two-Week Bulk-Move Sprint | Phase A | V5 | PLANNED — Day 10 (2026-07-15) | Sprint agent + Architect pairing |
| SPRINT-D7.1 | Fix ~72 test-project compile errors post-bulk-move: mechanical namespace sweeps for `LankaConnect.BuildingBlocks.Domain.Shared.*` → `SharedKernel.*` (Consult #13), delete `LankaConnect.Domain.Business.*` refs (Consult #12 Option D), retarget `ITicketService`/`IRefundExecutionService`/webhook-handler interfaces to `LankaEvents.Contracts/LegacyPromotions/` (Consult #17), Communications duplicate-using dedup. Blocks SPRINT-D10.1. | Sprint Day 7 slot A | Two-Week Bulk-Move Sprint | Phase A | V5 | PLANNED — Day 7 (2026-07-12) | Sprint agent |
| SPRINT-D7.0 | **BLOCKING Day 7 slot A entry point** — Consult #19 AppDbContext ownership-boundary hardening. Bulk `modelBuilder.Ignore<T>()` sweep for every LankaEvents/Communications/Identity/Forms/Media/Payments aggregate on AppDbContext (staging deploy currently red on VO discovery). Enumeration derived from Consult #7 Delta ownership matrix. Deliverables: (1) AppDbContext.OnModelCreating Ignore<T> block; (2) `docs/architecture/appdbcontext-ownership-boundary.md` (this document IS Rule 5b consult artifact per Consult #19 blanket pre-approval); (3) `tests/LankaConnect.Infrastructure.Tests/Data/AppDbContextModelParityTests.cs` asserting Ignore<T> list complete (T4/T-9 trigger, mirrors Rule 5e); (4) post-sprint ArchTest rule AppDbContext MUST NOT reach module-owned aggregate types except via Ignore<T>(). Unblocks 4C.h final close. | Sprint Day 7 slot A | Two-Week Bulk-Move Sprint | Phase A | V5 | PLANNED — Day 7 (2026-07-12) — highest priority | Sprint agent + Architect pairing |
