...
**Status**: ✅ **SHIPPED + STAGING-VERIFIED** (2026-05-01). Architect review iteration 1 applied (13 edits). 5 commits across 5 of 6 slices (7F-B.4 email template deferred as a post-ship follow-up since `notifyAttendees` defaults to false and the conversion already works without email — the audit table is the system-of-record).

**Commits**:
- `950bfea9` (7F-B.1) — domain: `Event.ConvertRegistrationMode` + `ConversionPolicy` + `ConversionReport`; 30 new tests covering A→B1/B2/B3/B4, B1/B2/B3/B4→A, Other-gender skip, named-seat skip, pending-addition skip, batch-cap reject, lead-name source, audit shape, A→B2-tiered with per-tier-age axis (depends on 7F-C live), B→A tiered placeholder allocation, stable sort by SortOrder, A↔C rejected, empty-event flips mode, all-adult/all-child collapses, live-tier-name preferred over snapshot, null-policy guard.
- `f89c5144` (7F-B.2) — persistence: `RegistrationModeConversion` (aggregate) + `RegistrationModeConversionRow` (per-row detail) audit entities + EF configurations + DbSets + auto-scaffolded migration `Phase7FB_AddRegistrationModeConversionAuditTables`. Required two scaffolding fixes (memory note: Designer.cs companion present per 6A.133; `configuredEntityTypes` allowlist gate at AppDbContext silently `Ignore()`'s any Domain class not in the array — the new entities had to be added there to prevent EF from skipping them at model build).
- `5bcf2a29` (7F-B.3) — application + API: `ConvertRegistrationModeCommand` + handler + `POST /api/events/{id}/convert-registration-mode` (Authorize, Owner-only). Architect Q8 wired: handler queries pending `RegistrationAddition` rows for the event upfront and passes the IDs into `ConversionPolicy.RegistrationIdsWithPendingAdditions`. On commit, persists one aggregate audit row + one detail row per migrated/skipped registration with BeforeShape/AfterShape jsonb snapshots. DryRun branch returns the report without persisting.
- `28d32dd7` (7F-B.5) — frontend: `ConvertRegistrationModeDialog` with diff preview powered by the dry-run branch + `useConvertRegistrationMode` mutation hook + EventEditForm wire-up (button appears beneath the picker when `currentRegistrations > 0` AND target mode differs from current). 4 new RTL tests covering: dry-run-on-mount call, Cancel doesn't commit, Confirm fires real commit + bubbles result, "all skipped → Confirm disabled".

**Tests**: 30 new domain + 4 RTL = 34 new across the slice. Application suite **2494 / 6 skipped / 0 failed** (+30 over post-7F-C 2464). Architect floor was ≥36; actual 30 in the domain layer alone.

**§6 staging end-to-end smoke (2026-05-01)**:
- ✅ **Migration applied**: psycopg2 confirms `__EFMigrationsHistory` carries `20260501030237_Phase7FB_AddRegistrationModeConversionAuditTables` and `events.registration_mode_conversions` + `events.registration_mode_conversion_rows` exist with the correct columns (event_id, organiser_id, from_mode, to_mode, total/migrated/skipped/failed counts, jsonb before/after shapes, FK cascade).
- ✅ **API endpoint registered**: differential probe — `POST /api/events/{id}/convert-registration-mode-DOES-NOT-EXIST` returns HTTP 404 (route not found), `POST /api/events/{id}/convert-registration-mode` returns HTTP 401 (route registered, `[Authorize]` gate firing). The 401 vs 404 differential proves the endpoint is reachable + the authorisation pipeline works as designed.
- ⏳ **Authenticated dry-run/commit smoke deferred**: same staging Auth bug as 7F-A §5 + 7F-C §5 — `/api/Auth/login` issues access tokens whose `iat`/`exp` JWT claims are anchored to 2026-04-25 11:46/12:16 UTC (5 days stale, immediately expired). Open endpoints work; `[Authorize]` endpoints reject HTTP 401 against the expired-on-arrival token. NOT a 7F-B issue. UI-driven smoke (cookie-session, not the broken JWT path) is the next step.

**Backend deploys**: `25201026645` (7F-B.3) `conclusion=success`. Frontend deploy `25201864033` (7F-B.5) success.

**Slice 7F-B.4 (email template `event-registration-format-changed` + handler with Hangfire fire-and-forget + dedupe) deferred** as a follow-up. The conversion works without email (audit table is the system-of-record); `notifyAttendees` defaults to false in both the API + UI; users get the toggle when 7F-B.4 ships.

**Next per architect ship order**: 7F-D (paid Mode B add-attendees with delta payment).
