...
## 🔄 CURRENT SESSION STATUS — PHASE 7D.1 PHASE E — FRONTEND TYPES + HOOKS + LABELS PROP (LOCAL READY)
**Date**: 2026-04-20
...
## 🔄 PARALLEL WORKSTREAM — WHATSAPP PREFERENCES SAVE 400 → 200 (DEPLOYING)
**Date**: 2026-04-20
**Session**: WhatsApp Fix #0 — Boundary normalisation fix to unblock `PUT /api/whatsapp/preferences` which was 400'ing whenever quiet-hours fields were left empty. Part of the larger WhatsApp RCA plate (remaining Fix 1+2+5 backend, Fix 3 UX, Fix 4 auto-disable).
**Progress**: ✅ **COMMITTED + PUSHED — commit `33ccc542` on develop.** Symptom: `"Request failed with status code 400"` on Save Preferences click whenever quiet-hours left empty. Root cause: `<input type="time">` submits `""`; `.NET TimeOnly?` model binding rejects `""` via `[ApiController]` auto-`ValidationProblemDetails`. Fix at the Zod validation boundary in [web/src/presentation/lib/validators/whatsapp.schemas.ts](../web/src/presentation/lib/validators/whatsapp.schemas.ts) — `nullableTrimmedString = z.string().optional().nullable().transform(v => v ? v : null)` applied to `quietHoursStart`, `quietHoursEnd`, `preferredLanguage`. Types split: `UpdatePreferencesFormInput` (`z.input<>`, what react-hook-form holds — may include `""`) vs `UpdatePreferencesFormData` (`z.infer<>`, post-transform — `""` → null). [WhatsAppPreferences.tsx](../web/src/presentation/components/features/whatsapp/WhatsAppPreferences.tsx) uses 3-generic `useForm<UpdatePreferencesFormInput, unknown, UpdatePreferencesFormData>` so form state accepts `""` but `handleSave(data)` receives the transformed nulls. **Tests**: new [whatsapp.schemas.test.ts](../web/tests/unit/presentation/lib/validators/whatsapp.schemas.test.ts) with 7 Vitest cases — `""`→null for each of the 3 fields, combined `""`×3, populated passthrough, explicit null, omitted undefined. RED→GREEN verified (7/7 pass, 9ms); `npx tsc --noEmit` zero errors. Deploy-ui-staging run `24696324247` currently in progress against `33ccc542`.
**Status**: 🚧 **DEPLOYING — AWAITING STAGING VERIFICATION.** Next: monitor deploy-ui-staging green → browser smoke on staging (enable WhatsApp → clear quiet-hours time pickers → click Save → expect 200 + persisted prefs) → curl smoke (`PUT /api/whatsapp/preferences` with `"quietHoursStart":""` body should succeed because the frontend never sends `""` anymore; direct null test: `"quietHoursStart":null` → 200). Then proceed to Fix 1+2+5 (backend `EffectivelyEnabled` alias + `WhatsAppSkipReason` enum + admin metric `usersEnabledButUnverified`) → Fix 3 (auto-request verification code on enable, persistent unverified banner profile-page-only) → Fix 4 (30-day grace period + scheduled auto-disable job + notification email).

---

## 🔄 PARALLEL WORKSTREAM — E1: ATTENDEE ADDRESS → OPTIONAL (CODE COMPLETE, READY FOR PR-A)
**Date**: 2026-04-20
...
**Status**: 🚧 **AWAITING COMMIT + DEPLOY + STAGING VERIFICATION.** Next: stage only the three code files + `docs/MASTER_TODO_E1_PHASE_C.md` + this doc + `PROGRESS_TRACKER.md` + `STREAMLINED_ACTION_PLAN.md` (explicit file-path `git add` — exclude `.claude/settings.json`, `.github/workflows/deploy-production.yml`, `web/tsconfig.tsbuildinfo`, and all parked Phase C files), commit as `fix(registration): E1 — make attendee address optional`, push `develop`. CI runs `deploy-staging.yml` + `deploy-ui-staging.yml`. Staging verification: token via `POST /api/Auth/login` (`niroshhh@gmail.com` / `1qaz!QAZ`) → `POST /api/events/{eventId}/register-anonymous` with no `address` key → expect HTTP 200 + registrationId. Browser smoke: open event registration form on staging, confirm label shows `(optional)` in light grey, submit with blank address, expect success. Azure container logs clean post-deploy. Only then start PR-B at C4.
