...
|---|---|---|---|
| **Wave 6.5.X.Y** | **MOVED to Wave 6.5** (per architect ruling 2026-06-30). Resolve Rule 5 debt: 14 `LankaConnect.Infrastructure` services/handlers/background-services directly reference `Products.LankaEvents.Application`. Cleanup pattern = publish integration events instead of invoking command/query types directly. Per blueprint §7.4 / D5. Affected: `RegistrationEmailService`, `PdfTicketService`, `TicketService`, `CsvExportService`, `ExcelExportService`, 7 `*WebhookHandler` classes, `RefundReconciliationBackgroundService`, `SeatHoldCleanupService`. Composition-root `DependencyInjection.cs` is permanently allowed (carries the runtime-vs-wiring distinction). | ⏳ PLANNED (Wave 6.5) | Coherent with Wave 6.5 Outbox cutover — integration-event mechanism required. Skip-fact currently on Rule 5. |
| **Wave 6.X.Z (Forms)** | ✅ **SHIPPED 2026-07-01 (Wave 6.a.1)** — 14 `Forms.Application` query types (7 queries + 7 handlers: GetPublicFormResponses, GetMyFormResponse, GetMyFormResponseByUserId, GetFormResponses, GetEventForms, GetEventFormDetail, ExportFormResponses) previously referenced `Products.LankaEvents.Application` directly. **RESOLVED** via: (a) shared DTOs (EventFormDto family + PublicFormResponseDto family + ExportResult + ExportFormat) moved from `Products.LankaEvents.Application.Common` to `Products.LankaEvents.Contracts`; (b) narrow `IFormResponseExporter` port added to Contracts so Forms doesn't need the full CSV/Excel service interfaces (which hold too many Product-internal signatures to publish); (c) `CsvExportService` + `ExcelExportService` in Infrastructure implement `IFormResponseExporter` alongside existing service interfaces; (d) Forms.Application.csproj now references `Products.LankaEvents.Contracts` only. Rule 9 flipped from Skip-fact to live Fact + passes. | ✅ CLOSED | |
| **Wave 6.5.X.W (Payments)** | **NEW Wave 6.5 debt** (surfaced during Wave 6.a.1 un-skip 2026-07-01). Resolve Rule 9b debt: 11 `Payments.Application` types (6 services + 3 event handlers + 2 command handlers: `AddOnRefundService`, `RefundExecutionService`, `RefundLineDispatcher`, `RefundReconciliationService`, `RefundTotalCalculator`, `RegistrationRefundService`, `PaymentCompletedEventHandler`, `RefundCompletedEventHandler`, `RegistrationPendingPaymentEventHandler`, `ApproveRefundRequestCommandHandler`, `CreateOrganizerInitiatedRefundCommandHandler`) directly reference `Products.LankaEvents.Application`. Structural twin of Wave 6.5.X.Y (Rule 5 debt) — same integration-event / Outbox cutover fix. Per architect ruling 2026-07-01 Option B: keep Skip-fact discipline visible, defer to Wave 6.5 alongside Rule 5 legacy services. | ⏳ PLANNED (Wave 6.5) | Coherent with Wave 6.5 Outbox cutover. Rule 9b Skip-fact tracks the 11 violators at ArchTest baseline. |

### Trigger-based conditional work (NOT scheduled waves)

Some architectural work fires only when a runtime pattern recurs — filing it as a scheduled wave sub-slice would imply a specific slot in the roadmap that doesn't exist. These items are tracked here + in memory rules; they escalate when their trigger fires.

| Item | Trigger | Notes |
|---|---|---|
| **Wave 7.X.R — Roslyn analyzer: EF shadow-metadata misuse family** | ✅ **TRIGGER FIRED 2026-07-01 (THIRD INSTANCE)** — architect ruling 2026-07-01 confirms F21 counts as the third instance of the EF shadow-metadata family (F17 / F18 / F21). Analyzer authoring is now justified but sequenced **after** Wave 9.h.10 email coverage closes (blocking) and Wave 6.5 Outbox cutover completes. | **Family shape (unified)**: developer references an EF metadata name (shadow property, shadow-table junction, or column string) that does not match the runtime CLR/model shape. Three concrete instances to date: <br>• **F17** (`98cc4e37`): `DbContext.Set<Dictionary<string,object>>("newsletter_metro_areas")` after junction was CLR-typed to `NewsletterMetroAreaLink`. <br>• **F18** (`98cc4e37`): same shape, `GetPublishedNewslettersQueryHandler`. <br>• **F21** (Wave 9.h.10.1, 2026-07-01): `EF.Property<string>(t, "category")` shadow-property access on a REAL VO-typed property (`EmailTemplate.Category` with `HasConversion`). Runtime failure = "property does not exist on entity type". <br>**Also tracked**: F20-shape (`.IsRequired()` on `Nullable<T>` domain property) still at 1 instance — deferrable until 2nd surfaces. <br>**Why deferred despite trigger firing**: Wave 9.h.10 email coverage is the current founder-critical work; Wave 6.5 Outbox cutover follows. Roslyn analyzer scoped in Wave 7.X.R against a stable post-6.5 codebase. <br>**Reference**: `[[roslyn-analyzer-recurrence-trigger]]` memory rule. |
