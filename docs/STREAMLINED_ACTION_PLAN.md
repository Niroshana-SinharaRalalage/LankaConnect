...
## 🔄 CURRENT STATUS — PHASE 7C.2b CHUNK 1: RE-APPLY DECOMPOSED LOCATION TO SIGNUP/VOLUNTEER COMMITMENT TEMPLATES (2026-04-23)
**Date**: 2026-04-23
...
**Status**: 🚧 **COMMITTED + DEPLOY IN FLIGHT** — commit `82d5f56f` on develop; `deploy-staging.yml` run `24811020806` in_progress. 21 new unit tests green; zero regression across all 4 non-Docker test suites. DB probe + live inbox smoke post-deploy.
...
**Next**: Post-deploy staging DB probe (expect 3 templates `has_new=true` + 2 cancellations unchanged) + live inbox smoke on event `d543629f` (commit to a signup item; confirm email shows Aurora Clubhouse bolded + 4314 Clark Ave Cleveland below it + Parking Lot block with Geoga Lake Parking Lot + 943 Penny Lane Aurora below THAT). If green: close Chunk 1 and proceed to Chunk 2 (7 registration/lifecycle templates requiring both code + body fix). If the migration aborts: inspect the `RAISE EXCEPTION` message for which invariant fired and which template, fix forward.
