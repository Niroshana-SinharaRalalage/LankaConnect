<#
.SYNOPSIS
  Smoke for RefundReconciliationController (Wave 9.e). 1 endpoint (admin).
#>
...
function Invoke-RefundReconciliationControllerSmoke {
    ...
    $allSections = @( @{ Name = 'refund-reconciliation'; Func = { Test-RefundReconciliationFlow -Report $report } } )
    ...
    $report = New-LcReport -Name 'Wave 9.e: Smoke-RefundReconciliationController'
...
if ($MyInvocation.InvocationName -ne '.') {
    $report = Invoke-RefundReconciliationControllerSmoke -Only $Sections -SkipLogChecksLocal:$SkipLogChecks
