...
User asked for a second main entry button on the umbrella landing page alongside LankaEvents, called LankaSeyla. Confirmed during brainstorming as an **external** storefront (`https://lankaseyla.lankaconnect.app/`, premium Sri Lankan clothing), opening in a new tab — not a new in-app module, so there is **no backend, API, EF or DB work in this item at all**.

**Shipped**: (1) NEW `scripts/generate-lanka-seyla-logo.js` — reproducible `sharp` pipeline producing `web/public/lanka-seyla.png` (443×110, wordmark trimmed and scaled to 110px height, natural width preserved); (2) NEW `EntryCard.tsx` in `presentation/components/features/landing/` — extracted from ~100 lines of inline JSX + four mouse handlers in `page.tsx` that each rebuilt a five-layer `boxShadow` from the brand colour, now one `shadowFor(state, color)` helper; props `brandColor`/`logoSrc`/`logoAlt`/`logoWidth`/`logoHeight`/`name`/`showName`/`badge`/`tagline`/`href`/`external`/`live`; (3) `page.tsx` middle band → `flex-col md:flex-row items-stretch` with both cards as `<EntryCard>`; (4) NEW `landing-entry-cards.test.tsx`.

**Design decisions worth carrying forward**: LIVE dot omitted on the external card (it signals live in-app data and would assert liveness we cannot observe); `showName={false}` for LankaSeyla because the wordmark already spells the brand out (name still reaches assistive tech via `alt`, which is what keeps the link's accessible name working); brand gold `#D7A959` sampled from the artwork; black logo background kept because LankaSeyla's own site header uses it.

**Pre-existing a11y gap fixed in passing**: the entry card is a `Link` wrapping a `div` with no focus style — keyboard-reachable but invisible when focused. `EntryCard` adds a `focus-visible` ring, repairing LankaEvents too.

**fix-1 (operator UAT)**: first revision cropped the 4.03:1 wordmark into a stacked 110×110 square lockup to preserve parity with the LankaEvents square-icon slot. Operator rejected it — parity was never a requirement. Wordmark now used as supplied; `EntryCard` gained `logoWidth`/`logoHeight` (default 55×55 so LankaEvents is untouched). A test pins the natural aspect (width 221 ≠ height 55) so a future square-crop regression fails loudly.

**Tests**: 12/12 in `landing-entry-cards.test.tsx`, 48/48 across `src/__tests__/pages/`, `tsc --noEmit` clean, `next build` compiled successfully. The four LankaEvents assertions were written and run BEFORE the extraction (red: 4 passed / 7 failed) so the refactor of working UI had to prove it changed nothing.

**CI gotcha discovered**: `deploy-ui-staging.yml` triggers on push to `develop` ONLY — merging to `Production_05_09_2026` does **not** auto-deploy; staging needs `gh workflow run deploy-ui-staging.yml --ref Production_05_09_2026`. Local `Production_05_09_2026` was also 117 commits behind origin and was fast-forwarded before merging.

**Spec**: [docs/superpowers/specs/2026-08-04-lankaseyla-landing-entry-design.md](./superpowers/specs/2026-08-04-lankaseyla-landing-entry-design.md). **Pending**: operator browser UAT (side-by-side layout at 320/768/1024px, 100vh hero not clipping the bottom tagline, new-tab behaviour).

---

## 🎯 2026-06-01 (Phase 6A.161 — Ticket Tier on the event-manage Attendees tab + CSV/Excel exports) — 🔧 CODE COMPLETE + TESTS GREEN + COMMITTED (`47ae66cf`); STAGING DEPLOY + API TESTS BLOCKED ON BRANCH COMPILE

User asked to display the ticket tier per attendee in the Attendees tab and add tier details to the CSV + Excel exports. **Architect-paired RCA classified as feature-missing (incomplete read-path) — NOT a DB, Auth, UI-only, or backend defect.** The denormalized `ticket_tier_name` is already persisted per attendee in the `registrations.attendees` JSONB (since migration `20260415203751`, 2026-04-15) — no join needed — but was dropped at the read path: projection ([GetEventAttendeesQueryHandler.cs:129-134](src/LankaConnect.Application/Events/Queries/GetEventAttendees/GetEventAttendeesQueryHandler.cs#L129)) mapped only Name/AgeCategory/Gender; shared `AttendeeDetailsDto` + `EventAttendeeDto` had no tier field; UI table and backend CSV/Excel exports never rendered it. **5 read-path edits, NO migration**: (1) nullable `TicketTierId`+`TicketTierName` on shared `AttendeeDetailsDto`; (2) map both in projection (no join) + tier-coverage log; (3) computed `EventAttendeeDto.TicketTierSummary` (uniform→name, mixed→`VIP, General`, none→`—`, never null/blank); (4) append one `Ticket Tier` column to BOTH `CsvExportService` + `ExcelExportService` (append-only, blank TOTAL-row cell, lockstep parity); (5) FE types (`ticketTierName`, `ticketTierSummary`) + Ticket Tier column in `AttendeeManagementTab.tsx` (collapsed summary + expanded per-attendee amber badge; colSpan 10→11 free / 13→14 paid). Nullable throughout (single-tier/free/legacy/Mode-B → `—`). Display model PO-confirmed (summary collapsed + detail on expand). **Tests**: 10/10 Infrastructure export tests GREEN (`Phase6A161TicketTierExportTests` — CSV+Excel parity, mixed-tier summary, legacy null→`—`, free-event column, append-position) + 4 line-ending regressions GREEN; frontend `tsc` clean. Application unit tests (`Phase6A161TicketTierSummaryTests`, 6 cases) authored; production code compiles but the `Application.Tests` project can't run yet — sibling session's in-flight `SponsorTests.cs` (6A.162 brochure WIP) has 23 compile errors referencing not-yet-added `Sponsor.BrochureUrl`/`SetBrochure` (not mine). **Shared branch `feat/phase-6a-162-sponsor-two-images` per user direction; surgical path-staging — committed only my 11 files, never the sibling's `otherprompts2`/`SponsorTests.cs`.** Build-env: concurrent `dotnet` builds in one worktree collide on `obj/bin` (OOM/stack-overflow/sourcelink locks) — worked around with a private `--artifacts-path`. **BLOCKED**: cannot deploy until the branch compiles (sibling WIP); then `deploy-staging.yml` + `deploy-ui-staging.yml` together → staging API tests T1–T7 → re-run `Application.Tests`. **Master TODO**: [docs/MASTER_TODO_PHASE_6A_161_ATTENDEE_TICKET_TIER_2026_06_01.md](./MASTER_TODO_PHASE_6A_161_ATTENDEE_TICKET_TIER_2026_06_01.md).

---

## 🎯 2026-06-01 (Phase 6A.157-fix-1 — three operator UAT findings on the deployed 6A.157 buyer flow: retire in-registration sponsor block, add buyer logo upload to purchase modal, collapse custom-amount form when packages present) — ✅ 3 COMMITS PUSHED; UI DEPLOY PENDING; OPERATOR RE-UAT PENDING

Operator UAT after 6A.157 deployed + smoke 4/4 GREEN: (1) retire in-registration sponsor block (ticketing complexity with combined Stripe sessions), (2) no way to add image for a package sponsorship (FE feature-missing — backend [2/6] deliberately shipped `sponsorId` for this purpose, [5/6] modal never wired the picker), (3) collapse "Or choose your own amount" by default (progressive disclosure — packages are primary CTA). Architect classified all three as **UI-only** (NOT backend/API/EF/DB/feature-missing in architecture sense). **3 commits on `feat/phase-6a-157-sponsorship-public-purchase`**: (1) `fded3a4f` retire in-registration sponsor block (UI-only removal; backend optional fields preserved for backward-compat); NEW `EventRegistrationForm.test.tsx` 2/2 GREEN narrow regression net. (2) `098cc8cf` buyer logo upload in `PurchaseSponsorshipPackageModal` (reuses `useUploadSponsorImage`, best-effort between `mutateAsync` resolve and `location.assign`, non-fatal on failure per architect decision); 5 new RTL tests, modal now 19/19. (3) `c30169eb` default-collapse custom-amount form via IIFE+nested CollapsibleSection when packages render alongside; render directly when no packages (form is only surface); 4 new SponsorSection tests, now 10/10. **Tests GREEN**: 192/192 events vitest (was 181 pre-fix-1; +11 new, zero regression) + TypeScript clean + Next build successful. **No backend deploy** — UI-only. **Pending**: `deploy-ui-staging.yml` → operator re-UAT (Stripe test card 4242 4242 4242 4242 + confirmation email + regression check existing custom-amount sponsor flow). Per memory `feedback_word_shipped`: flips to "shipped" only after operator signs off.

---

## 🎯 2026-05-31 (Phase 6A.157 — Sponsorship Packages public purchase flow; buyer-facing surface enabling anonymous visitors to purchase organizer-defined packages via Stripe Checkout) — ✅ 6 COMMITS PUSHED; DEPLOY PENDING

Closes the buyer-side gap left open by 6A.156. Architect-paired RCA across 4 passes (3 substantive + validation correction) classified as a **feature-missing case** for the buyer side; backend/API/EF schema from 6A.156 is sufficient (no new migration beyond the [3/6] email template seed). Branched off `main` as `feat/phase-6a-157-sponsorship-public-purchase`. **6-commit sequence**: (1) `101c73f1` domain — Sponsor.CreatePackageSponsor factory + CompletePackagePayment + CompleteAsOrganizerCash mutual guard + new PackageSponsorCompletedEvent; (2) `2c508d2c` app+infra — CreatePackageSponsorCommand (handler mirrors PurchaseAddOnCommandHandler byte-for-byte: validate event/sponsors/EnablePackages, atomic TryReserveStockAsync, Stripe session OR free $0 instant complete, idempotent stock restore on any failure) + GetActiveSponsorshipPackagesQuery (returns [] for non-opted-in events, never errors) + SponsorshipPackagePublicDto + IStripePaymentService.CreatePackageSponsorCheckoutSessionAsync (literal `payment_type: "package_sponsor"` at all 3 metadata sites + dispatcher case, byte-identical so no metadata-loop overwrites) + PackageSponsorWebhookHandler (Completed → CompletePackagePayment; Expired → MarkAsAbandoned + TryRestoreStockAsync; rejects misrouted webhooks) + DI + PaymentsController switch cases; (3) `a7fd26bc` email — NEW migration `Phase6A157_AddPackageSponsorEmailTemplate` + PackageSponsorConfirmationEmailParams + PackageSponsorCompletedEventHandler subscribing to the new domain event; (4) `04949f4e` API — SponsorshipPackagesController gains 2 `[AllowAnonymous]` endpoints (`GET /active`, `POST /{packageId}/purchase`) + CreatePackageSponsorRequest DTO + 7 reflection-based contract tests (route templates + [AllowAnonymous] gate + DTO shape, caught at build time per memory `feedback_401_does_not_prove_feature_reachable`); (5) `6f21faf8` FE — 3 types + 2 repo methods + 2 hooks (`usePublicSponsorshipPackages` with separate `publicList` cache key + `usePurchasePackageSponsor`) + NEW portal'd `PurchaseSponsorshipPackageModal` (form-nesting safe per 6A.156-fix-2 contract + stopPropagation belt-and-suspenders + redirects via `window.location.assign(checkoutUrl)` — Stripe for paid, SuccessUrl directly for free $0) + NEW `PublicSponsorshipPackageCard` (display-only sibling of organizer Card: no edit/delete/image-upload; sold-out → disabled + badge for defense-in-depth; "N left" warning at remainingStock ≤ 5) + 14 modal RTL tests; (6) `05a1b7ee` SponsorSection integration — package grid (1/2/3 responsive) ABOVE the existing mode toggle + "Or choose your own amount" divider + modal mount; two-layer gate (`enabled` hook flag + `length > 0` DOM check) so non-opted-in events see the original UI unchanged — **zero regression risk for existing custom-amount sponsor flow**; 6 new SponsorSection regression tests. **Tests GREEN**: Application 2846/2852 + Infrastructure 354/354 (matches [2/6] +5 query +8 webhook baselines) + controller reflection 7/7 + all 20 events vitest files **181/181 GREEN** (zero regression, +27 new test cases this phase: 14 modal + 6 SponsorSection + 7 reflection) + TypeScript 0 errors + Next.js production build successful + `dotnet build` clean. **2 Domain failures** (FormResponseTests.UpdateAnswer_Should_Succeed + DonationConfigurationTests.Create_WithMinGreaterThanMax_Should_Fail) are pre-existing on the branch, unrelated (no FormResponse/DonationConfiguration touched). **14 handler tests for CreatePackageSponsorCommandHandler DEFERRED** per [2/6] — real Event-aggregate fixtures prohibitively expensive without an Event test-builder; handler's sequential logic mirrors AddOn byte-for-byte and gets validation from staging API smoke post-deploy. **Pending**: deploy backend `deploy-staging.yml` + UI `deploy-ui-staging.yml` in same chain per memory `feedback_deploy_backend_and_ui_together` (branch touches both src/ and web/). Staging API smoke per memory `feedback_post_deploy_api_test`: auth + `GET /sponsorship-packages/active` 200/array + `POST /{packageId}/purchase` 200 + `{ checkoutUrl, sponsorId }` + 409 on sold-out. Operator browser UAT: public event detail page → package cards above custom-amount form → click → portal'd modal → fill form → Stripe redirect → simulate webhook → confirm email arrives with package details + conditional included-tickets note.

---

## 🎯 2026-05-30 (Phase 6A.156-fix-3 — show Sponsorship Packages table on Event Details tab; corrective UI display gap after operator UAT on 6A.156-fix-2) — ✅ COMMITTED + PUSHED (c12b1630); STAGING-UI DEPLOY DISPATCHED (run 26699318913); OPERATOR RE-UAT PENDING

Operator UAT on the now-deployed 6A.156-fix-2 confirmed the modal flash bug is RESOLVED (3 packages created: Platinum/Gold/Song). They raised TWO new gaps: (1) buyer flow on public event detail page can't select packages — classified as **feature-missing** (6A.157 scope, explicitly deferred at 6A.156 design time; needs its own RCA + fresh branch + multi-day vertical slice with payment-intent + ticket bundling + atomic stock decrement + concurrency guard — DO NOT BUNDLE); (2) Event Details tab's Sponsor Configuration section is missing the Sponsorship Packages table that mirrors the Add-On Items table under Add-On Configuration — classified as **UI display gap**, hours of work, single file, lowest-risk delivery profile of the series. Architect locked Option A: ship Gap 2 now, defer Gap 1 to 6A.157. Backend verified healthy: API returns `enablePackages: true` + 3 packages for event `ad8903c4-…`. **What 6A.156-fix-3 ships**: [EventDetailsTab.tsx](web/src/presentation/components/features/events/EventDetailsTab.tsx) gains `useSponsorshipPackages(event.id, enabled)` with two-layer gate (`isEnabled === true AND enablePackages === true`) so no fetch fires for sponsor-disabled/packages-disabled/legacy events. New "Sponsorship Packages" inline table at bottom of Sponsor Configuration CollapsibleSection, byte-for-byte mirror of Add-On Items table at lines 924-984. Columns: Name + Tier badge + description / Price (or "Free" badge) / Stock (sold/limit or "Unlimited") / Tickets (`N tickets` or em-dash) / Perks (`N perks` summary or em-dash) / Status (Active/Inactive badge). Empty state matches add-on copy. Header uses `Award` icon (amber) for visual parity with editor. Inlined per architect — symmetry beats DRY; Add-On Items is inlined too. **Tests GREEN**: NEW `EventDetailsTab.test.tsx` (4 cases) pinning hook-gating contract (enabled=TRUE both gates / enabled=FALSE packages off / enabled=FALSE sponsors disabled / enabled=FALSE sponsorConfig null). Visual/HTML verified by operator UAT (Add-On Items table has zero unit tests today — follow codebase convention). Existing tests untouched: **33/33 GREEN** (EventDetailsTab 4 + SponsorshipPackageEditor 15 + SponsorshipPackageEditModal 3 + SubConfigForms 11). TypeScript clean. Next.js build clean. **No backend / API / EF / migration changes**. **Operator re-UAT pending**: staging-ui `/events/{id}/manage` → Event Details tab → expand Sponsor Configuration → confirm "Sponsorship Packages (3)" table renders with Platinum/Gold/Song rows. **Reserved as 6A.157**: Public Purchase Flow (buyer-facing package picker on event detail page, atomic POST endpoint with stock decrement + ticket bundling + perks claim row + Stripe payment-intent integration, concurrency guard via row-version, ~3-5 days, needs its own master TODO + index reservation + fresh branch when user is ready).

---

## 🎯 2026-05-30 (Phase 6A.156-fix-2 — modal flashes + boots operator from edit mode; corrective work on 6A.156-fix after operator clicked "Add Package" during post-deploy UAT) — ✅ STAGING-DEPLOYED + OPERATOR-VERIFIED 2026-05-30 (operator confirmed 3 packages created via inline editor)

Operator clicked "+ Add Package" inside the just-deployed inline editor in EventEditForm. Modal flashed open then immediately disappeared and the operator was kicked out of edit mode back to the manage page. Verbatim: "When I try to create a package, it opens up a modal pop and closes it suddenly and get out from the event edit mode." Architect-paired RCA classified as a **UI bug** (NOT Auth/Backend/API/EF/DB/feature-missing). Backend/API/EF/migration from 6A.156 stay untouched. **Two interlocking causes**: (a) `Button.tsx` (ui/) spreads `{...props}` onto a native `<button>` without defaulting `type` — per HTML spec, an unmarked `<button>` inside a `<form>` defaults to `type="submit"`. My editor's "Add Package" used `<Button onClick={...}>` with no explicit type → click cascade: `setModalOpen(true)` fires (modal flashes) THEN react-hook-form's `handleSubmit` validates → PUT `/events/{id}` succeeds → component redirects to manage page → EventEditForm unmounts → modal vanishes mid-render. Codebase-wide risk: 447 `<Button>` callers, only 23 with explicit `type=` — ~424 latent submit-on-click sites. (b) `SponsorshipPackageEditModal` rendered its OWN `<form>` while DOM-nested inside EventEditForm's `<form>` (modal was `fixed`-positioned but NOT portal'd — `fixed` is visual only, DOM ancestry unchanged). Nested `<form>` is invalid HTML; browsers silently strip the inner tag. **4 fixes locked by architect, one surgical commit `2f5da83c`**: (1) `Button.tsx` defaults `type` to `"button"` (audit confirmed login/register/reset-password use raw `<button type="submit">`, NOT the `<Button>` component — zero auth regression risk); (2) `SponsorshipPackageEditModal` wraps root in `createPortal(..., document.body)` with `mounted`+`useEffect` guard for Next.js SSR safety; (3) belt-and-suspenders explicit `type="button"` on editor's 3 trigger buttons + modal Cancel; (4) `AddOnDefinitionEditor.tsx:411` "Create Add-On" trigger had the exact same latent bug — same one-line fix (user said "behavior is add-on", keep flows consistent). **TDD regression tests** load-bearing for the entire bug class: editor test +3 cases (15 total) render inside `<form onSubmit={spy}>` and click each CTA, assert spy NOT called AND modal opened; NEW modal test +3 cases render modal inside `<form>`, fill + click Create/Cancel/Close, assert spy NOT called AND right callback fired. **29/29 GREEN** + TypeScript clean + Next.js build successful. **No backend / API / EF / migration changes**. **Operator re-UAT pending**: deploy-ui-staging run `26696508004` completion → click "+ Add Package" in inline editor → confirm modal opens cleanly + stays open + edit mode preserved → fill fields + submit → confirm package persists + modal closes → regression-check EventCreationForm flow.

---

## 🎯 2026-05-30 (Phase 6A.156-fix — Sponsorship Packages container refactor; corrective work on 6A.156 after operator UAT flagged the organiser CRUD surface placement as wrong) — ✅ STAGING-DEPLOYED (run 26695653565); SUPERSEDED BY 6A.156-fix-2 (the modal-flash bug above surfaced during this fix's UAT)

Operator UAT verbatim: "We don't need a separate tab called package. You can include them inside the sponsor tab. Other thing is not only the enabling the sponsorship package, but also, create them should go to event create/edit page as we can create/edit add-ons. the behavior is add-on, which I mentioned already." Architect-paired RCA classified as a **UI placement defect — frontend-only refactor** (NOT feature-missing, NOT backend/API/EF/DB/Auth). Original 6A.156 backend, EF migration, REST endpoints, and `EnablePackages` flag stay correct and unchanged. **Root cause**: I built a delivery-equivalent surface (CRUD that works) rather than a shape-equivalent surface (CRUD that lives where add-ons live). The user said "behavior is add-on" twice (initial RCA + delta) and I described the add-on pattern at the data layer but never enumerated the component composition: `AddOnConfigForm` wraps `AddOnDefinitionEditor` (at `AddOnConfigForm.tsx:129`), the editor has a `pendingDefinitions` local-mode for pre-event-creation use, and the same editor is reused inside `AddOnsManagementTab.tsx:184` for post-creation live CRUD — same component, two mount sites. I should have run `grep -n "AddOnDefinitionEditor" web/src` before writing a single line of package UI. Instead I built a live-mode-only hook layer + standalone `SponsorshipPackagesManagementSection` mounted in a brand-new "Packages" sub-tab, which structurally precluded pre-event-creation editing. Also: sub-tabs in `AttendeesAndFinanceTab` are **revenue categories** (Tickets/Donations/Collections/Sponsors/Add-Ons) — packages aren't a peer category; they're a sub-shape of Sponsors (a package purchase produces a Sponsor record). Promoting Packages to a peer sub-tab broke that mental model. **3-commit fix series on `feat/phase-6a-156-sponsorship-packages-foundation`** (same branch, no rename): (1) `1889d4b2` foundation — NEW `SponsorshipPackageEditor.tsx` (dual-mode, `eventId?` gates live-vs-local; reuses SponsorshipPackageCard + SponsorshipPackageEditModal as primitives byte-for-byte; live mode uses existing React Query hooks; local mode dispatches into parent-owned `PendingSponsorshipPackage[]` via `onPendingPackagesChange`) + modal `onSubmitOverride` prop so local mode intercepts submit + new exported `PendingSponsorshipPackage` interface mirroring `PendingAddOnDefinition` + 12 TDD test cases (12/12 GREEN). (2) `067a5d48` wire-up — SponsorConfigForm gains optional `eventId`/`pendingPackages`/`onPendingPackagesChange` props + embeds the editor inside the `{isEnabled && enablePackages}` block (mirrors `AddOnConfigForm:129`); toggle helper text dropped the "Packages tab" copy; EventCreationForm adds `pendingSponsorshipPackages` state + per-item POST loop after event create (mirrors add-on loop at `EventCreationForm.tsx:606-625` byte-for-byte, non-blocking on failure); EventEditForm single-line `eventId={event.id}` so embedded editor runs in live mode; SubConfigForms.test.tsx +3 cases asserting editor presence by data-testid (NOT rendered when sponsors off; NOT when packages off; rendered when both on) — 11/11 GREEN. (3) `7f998325` cleanup — DROP the `'packages'` SubTab union member + SUB_TABS entry + case branch + import from AttendeesAndFinanceTab; EMBED `<SponsorshipPackageEditor eventId={eventId} />` (live mode) inside SponsorsManagementTab between Sponsor Settings card and Summary cards, gated on `sponsorConfig?.enablePackages === true` (non-packages events see original Sponsors UI completely unchanged — zero regression risk); DELETE the now-orphaned `SponsorshipPackagesManagementSection.tsx`. **Tests GREEN**: TypeScript `tsc --noEmit` clean + vitest 23/23 (SubConfigForms 11/11 + SponsorshipPackageEditor 12/12) + Next.js production build successful. **No backend/API/EF/migration changes** — pure frontend refactor; original 6A.156 staging API smoke 6/6 remains valid. **Operator browser UAT pending**: deploy-ui-staging run `26695653565` → staging-ui `/events/{id}/manage` → Attendees & Finance tab → confirm Packages sub-tab is GONE → toggle "Enable sponsorship packages" inside the Sponsors sub-tab Settings card → confirm new editor section appears inline below settings → create/edit/upload-image/delete a package via the inline editor → ALSO test EventCreationForm + EventEditForm inline package editor flows → regression-check existing custom-amount sponsor flow on public page is 100% unchanged.

---

## 🎯 2026-05-30 (Phase 6A.156 — Sponsorship Packages Foundation — Phase A+B+C bundle of 5-slot block 6A.156→6A.160) — ✅ STAGING-DEPLOYED + API-VERIFIED 6/6 GREEN; SUPERSEDED BY 6A.156-fix container refactor (UI placement only)

User asked for the platform to support organizer-defined sponsorship packages (Gold/Silver/Bronze tiers) modelled after the Add-on flow, while keeping the existing free-form custom-amount sponsorship working. Architect-paired RCA across 2 passes classified as **feature-missing** (not UI/Auth/Backend-API/DB) — original `Sponsor` aggregate is a flat type-discriminator (Money|Item) that can't express tiers/perks/stock-caps/included-tickets without a second aggregate. Add-ons solved this with `AddOnDefinition` + `AddOnPurchase`; we lift that pattern. **4 user-locked decisions** asked mid-RCA via AskUserQuestion: (a) perks informational v1; (b) **include ticket count YES** (deviates from architect recommendation — adds `IncludedTicketCount` int 0-20; ticket allocation lands in 6A.158); (c) **reuse existing `SponsorSection`** for buyer-facing UI (no new public section — packages render as cards above the existing custom-amount form in 6A.157); (d) **bundle A+B+C into one PR** for visible value (3.5 days, not schema-only). Capacity-race edge case (6A.158): partial-fulfillment (sponsor keeps Completed, no tickets, buyer notified). **5-slot block reserved 6A.156→6A.160** (foundation / public purchase / ticket bundling / RSVP bundling / tier grouping). **What 6A.156 ships**: NEW `SponsorshipPackage` aggregate at [SponsorshipPackage.cs](src/LankaConnect.Domain/Events/SponsorshipPackage.cs) — Name(≤200), Description(≤1000), Money Price (≥0), QuantityLimit?, QuantitySold (atomic SQL), IsActive, SortOrder, ImageUrl/BlobName, Tier (free-text ≤100), Perks (Postgres `text[]`, max 10×200), IncludedTicketCount (0-20); methods Create/UpdateDetails/Deactivate/Activate/HasAvailableStock/RemainingStock/SetImage/ClearImage. Sponsor aggregate gains 7 nullable additive fields (no factory change, no behaviour change). SponsorConfiguration VO gains `EnablePackages` (default false; JSONB backward-compat via 6A.129 ValueComparer). Single EF migration `Phase6A156_AddSponsorshipPackages` creates `events.sponsorship_packages` (16 cols + 2 indexes + CASCADE FK) + 7 nullable cols on `events.sponsors` + 2 partial indexes + CHECK constraint `chk_sponsors_package_snapshot` + 2 SetNull FKs. `ISponsorshipPackageRepository` impl lifts atomic-stock raw SQL byte-for-byte from `AddOnDefinitionRepository`. Full application layer (Create/Update/Delete/SetImage/ClearImage commands + GetEventSponsorshipPackagesQuery + DTO + handlers — all with Serilog LogContext + Stopwatch + try/catch per CLAUDE.md observability). NEW `SponsorshipPackagesController` at `api/events/{eventId}/sponsorship-packages` — 6 organizer-only endpoints (public read + buyer purchase land in 6A.157). SponsorConfigurationDto + UpdateSponsorConfigCommand + EventConfigController + EventCreation/EditForm callers all extended with optional `EnablePackages` (backward-compat default false). **Frontend organiser UI**: 3 new components (Card / EditModal with client-side validation mirroring domain constants / ManagementSection with empty state + confirm-dialog deletes + self-gating); new `useSponsorshipPackages` hook with 6 React Query hooks; NEW "Packages" sub-tab in `AttendeesAndFinanceTab` between Sponsors/Add-Ons (existing SponsorsManagementTab 100% untouched — zero regression risk to existing UI); `EnablePackages` toggle wired through SponsorConfigForm + both Create/Edit forms. **Project-specific gotcha resolved**: `AppDbContext.IgnoreUnconfiguredEntities` allowlist needed `SponsorshipPackage` typeof entry — initial migration silently dropped the new table ("first mapped explicitly and then ignored"); added entry next to AddOnPurchase. **Tests GREEN**: 56/56 new SponsorshipPackage domain tests + 213/213 Sponsor+Config tests (zero regression) + Application 2822/2828 + Infrastructure 346/346 + frontend SubConfigForms 8/8 + TypeScript 0 errors + Next.js build clean (`✓ Compiled successfully in 34.0s`). **Deploys**: backend run `26690514756` GREEN + frontend run `26690515493` GREEN. **API smoke 6/6 GREEN** via `/tmp/verify_6a156.sh` end-to-end (auth → list HTTP 200 confirms migration applied and table accessible → create returns new ID → update + GET round-trip confirms `tier='SmokeTier2'` + `priceAmount=150.0` + `includedTicketCount=3` + `perks=['updated perk']` all persist correctly — Postgres `text[]` array works → delete returns 204). **3 commits on `feat/phase-6a-156-sponsorship-packages-foundation`** (branched off `main 7f04ef1d`, independent of in-flight 6A.154): `c3c422a1` domain+migration+tests / `59cf457e` app+API / `bcf0c299` frontend. **Operator browser UAT pending**: staging-ui `/events/{id}/manage` → Attendees & Finance tab → enable "Sponsorship Packages" toggle in Sponsor settings → click "Packages" sub-tab → create/edit/upload-image/delete a package + verify existing custom-amount sponsor flow on the public page is 100% unchanged (regression check). Next phases reserved: 6A.157 public purchase flow, 6A.158 ticket bundling, 6A.159 RSVP bundling, 6A.160 tier grouping.
