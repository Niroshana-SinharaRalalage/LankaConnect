...
- **UX follow-up 5 — optimistic reorder hits kind-filtered cache:** ✅ commit `7f192917`; `deploy-ui-staging.yml` run `24791468838` green. Organizer re-reported after UX #4 shipped: "It takes about 4 seconds to move one item up/down with the arrow button click." UX #4 unlocked the buttons (clicks land) but the visible reorder still took the full PUT + refetch. Root cause: Phase 7D.1 (`57437029`) kind-filtered query keys — `SignUpManagementSection` subscribes via `useEventSignUps(eventId, kind)` caching under `['signups', 'list', eventId, { kind: 'Items' }]`, but `useReorderSignUpItems.onMutate` wrote to exact-match `setQueryData(signUpKeys.list(eventId), ...)` — the unfiltered key that no component was subscribed to. UI only updated after `onSettled`'s prefix-match invalidation forced a full refetch. Fix: swap `getQueryData`/`setQueryData` for prefix-match `getQueriesData`/`setQueriesData` with `{ queryKey: signUpKeys.list(eventId) }` — covers both unfiltered and any kind-filtered entries instantly; `onError` rollback now iterates all restored tuples. 4 TDD tests added in new `useReorderSignUpItems.optimistic.test.ts`: bug-demonstrating kind-filtered update + unfiltered regression guard + both-variants + error-rollback-all — all green; `tsc --noEmit` clean.
- **Human-gated gap:** browser/mobile/keyboard smoke on staging UI for (a) reorder now feels instant (no PUT-wait), (b) arrow-button responsiveness during rapid clicks, (c) tab-stickiness across reorder — cannot be automated from CLI.
