...
| 6A.152 | `/events` page Upcoming/Completed split is now **date-based, not status-based**. Backend `EventStatusFilter.Active` returns events with `StartDate >= now OR StartDate IS NULL`; `EventStatusFilter.Inactive` returns events with `StartDate < now`. Both buckets exclude `Cancelled`/`Draft`/`UnderReview`. Frontend drops the client-side `status === Completed` filter (lines 131-137) and the `hasCompletedEvents &&` heading gate (line 444); Completed section always renders with an empty-state card. Fixes prod symptom: only ~3-4 upcoming cards shown and no Completed section, caused by historical Published-status events whose date passed but were never auto-flipped to Completed (Hangfire `EventStatusUpdateJob` lacks a `Published → Completed` shortcut — orthogonal concern, deferred). No background-job change, no migration, no data backfill | ✅ STAGING + PROD DEPLOYED (PR #128 → Production_05_09_2026, PR #129 → main); operator UAT pending | See [MASTER_TODO_PHASE_6A_152_EVENTS_DATE_BASED_FILTER_2026_05_24.md](./MASTER_TODO_PHASE_6A_152_EVENTS_DATE_BASED_FILTER_2026_05_24.md); revises the status-based split shipped in 6A.149 | 2026-05-24 |
| 6A.153 | Organizer-controlled Registration Window — turn registration on/off per event independent of publish state. Domain gets two new nullable UTC fields (`RegistrationOpensAt`, `RegistrationClosesAt`) + `SetRegistrationWindow` mutator + window guards in all 3 `Register*` methods (defense-in-depth). EventDto adds raw timestamps + computed `RegistrationAvailability` string (`"open"`/`"not-yet-open"`/`"closed-by-organizer"`/`"closed-event-started"`) — separate from existing `RegistrationModeStatus` to preserve its Phase 7E semantics. Frontend: click-through `RegistrationOpensSoonModal` for `not-yet-open` (Register button stays rendered, click reveals modal with formatted local time); inline "Registration is closed" microcopy for `closed-by-organizer`; organizer create/edit forms get 2 datetime pickers under "Registration Window (Optional)" with `ClosesAt` behind "Advanced" disclosure. One EF migration adds the two columns (nullable `timestamp with time zone`) + indexes; no backfill (null = always open, backward-compatible). No separate "pause now" toggle — close immediately = set `ClosesAt = now`. Branched off `main` (post-6A.152 sync); does NOT ride the 148/149/150/151 production bundle | 🔧 In Progress | See [MASTER_TODO_PHASE_6A_153_REGISTRATION_WINDOW_2026_05_25.md](./MASTER_TODO_PHASE_6A_153_REGISTRATION_WINDOW_2026_05_25.md); architect-approved 11-decision plan + 27-case test matrix | 2026-05-25 |
