...
**Phase A**: 🎉 **CLOSED 2026-07-15** at head `f3033074` on `develop` (tag `phase-a-close`). **Phase A STABILIZATION CLOSED 2026-07-19** at head `baa373aa` on `develop` per Wave 5 verification pass. Evidence: `docs/sprint/PHASE_A_STABILIZATION_CLOSED.md`. Wave 8.5 debt catalog: 10 of 12 items DELIVERED (a Part 1-4 + b Part 5 + d + f + g + h + i + j + k + l), 2 carry-forward (c ApiRename bounded per Consult #27 Q4.b; e workflow-restore tail blocked on IEventRecommendationEngine.cs GAP-1 residual). Wave 9 smoke moved 291/21/88 (72.75%) → 348/6/35 (89.46% pass) at Wave 5 fresh run (`reports/wave-9-20260719-173313/INDEX.md`, +57 pass / -15 fail / -53 skip / +16.71 pp). ArchTest 49/0/9 → 51/0/10 (+2 pass, 0 CI-blocking). Consult #28 R1-R5 all CLOSED or de-escalated. Phase B FULL kick-off (scaffolding + first-slice + cross-module writes) NOW UNBLOCKED — pending founder ratification of Wave 5 briefing pack (D2 review + D3 readiness + D5 risk matrix + D6 sequencing) + R6/R7/R8 disposition per `docs/coordination/RISK_MATRIX_2026_07_19.md`.

*Latest (2026-07-19, Phase A STABILIZATION CLOSED at head baa373aa, Wave 5 verification pass)* — **COMMITTED TO DEVELOP** as final Phase A stabilization close-out. Wave 5 Agent-Verification ran fresh Wave 9 smoke against last successful staging deploy `b91e6c10` (2026-07-18 11:54 UTC — deploy trail RED since due to IEventRecommendationEngine.cs referencing 5 types deleted by GAP-1 Part A `302af044`) + fresh ArchTest run (51/0/10/61 in 29s) + doc reconciliation. Wave 8.5 aggregate delivery summary:
- **8.5.a Part 1+4** — LankaConnect.Application csproj DELETED at `2f0f257d` per Tech Lead D-12 Option b DTO reshape (`bcf435c6` AccessTokenClaims + IJwtTokenService reshape + 3 caller cutover); Parts 2+3 (`c2a6e3fc` Dashboard fold-in + MetroArea profile relocation); ArchTest fixups `4cd93606` + `3df153f1`.
- **8.5.b Part 5** — 5 physical relocations shipped (Class1 delete + Security to Identity.Infra + TimeZone to LankaEvents.Infra + Validation to LC.API + Templates to Communications.Infra). 250 legacy migrations permanent-defer under AppDbContext per Consult #26 Q4 (`docs/coordination/migration-audit-2026-07-16.md`).
- **8.5.c** — DEFERRED per Consult #27 Q4.b bounded blast radius; no Phase-B block. Queued behind Wave 5 founder ratification.
- **8.5.d** — LegacyPromotions folder split (Media `ba25bc4e` + Communications `2aed1ded` + LC.API DI cutover `910dc7a9`).
- **8.5.e** — Test-project compile-fix Batches 1-3+ shipped (`a2eacbd8`, `8d73ec3e`, `90fc2bfd`) + LayerInversion aftermath 6 SharedKernel PR additions (`998fb58e`→`fcbe5aef`). Full-solution deploy-staging.yml restore BLOCKED on IEventRecommendationEngine.cs residual.
- **8.5.f** — DomainEventSaveChangesInterceptor wired on ALL module DbContexts at `dcd6c492` (initial 3 modules `1212d994`). **Consult #28 R1 CLOSED** — LIVE production silent-dispatch-drop resolved.
- **8.5.g** — ~116 LankaEvents.Application handlers migrated direct-SaveChanges across 9 commits (HandlerMigration-A 5 batches: `eaea551d`+`5192553a`+`451248b4`+`c50b434d`+`5e71f09e`+`bb6f7d35`; HandlerMigration-B 3 batches: `3c4ed694`+`04418850`+`c66e1607`; HandlerMigration-C 3 batches: `5727cf43`+`9b3c1b8a`+`1c927152`). **Consult #28 R2 CLOSED** — write-loss surface reduced H → L.
- **8.5.h** — `IMultiContextUnitOfWork.CommitAsync(DbContext[])` retired per Tech Lead D-01 (`2d296aca` + `6b4b4676`). Per-context SaveChangesAsync + saga-later disposition.
- **8.5.i** — `IIdentityMetroAreaJunctionRepository` authored + RegisterUser + UpdatePreferredMetroAreas cutover (`7e98bf94` + `b6a576d3`). Zero raw-SQL cross-module writes in Application layer.
- **8.5.j** — Currency-object JSONB normalization migration (`31e2ac41` + `ff02b13b`) + ADR-007 JSON-column VO standardization (`bffbb357`). 5 money-flow residuals ISOLATED per Consult #28 Q3.b as Phase-A-residual. **Consult #28 R3 ISOLATED + ADR-LOCKED.**
- **8.5.k** — Businesses controller REMOVED per Tech Lead D-07 founder direction (`c9df3599` + `c7a9dbd7`).
- **8.5.l** — PhotoAlbums verify-only — passed via 8.5.f + 8.5.g work.
- **Consult #28 R4 CLOSED** — SKIP audit `1ffee920` + ResidualFails cascade dropped skip count 88 → 50.
- **Consult #28 R5 CLOSED** — Docs refresh `b320c6ce` + this Wave 5 reconciliation commit.

Extras shipped during sprint window: **GAP-1** CulturalCalendar retirement + real PoyaCalendarService (`302af044` + `4bef04cf`); **GAP-6** layer inversion — Address/GeoCoordinate/Email/PhoneNumber promoted to SharedKernel.Contact + SharedKernel.Geo (`839fec4a`+`d13e2b0b`+`ff5d4762`+`0eced7b5`); **Phase B scaffolding** LankaTemples product shell (`36d1fce2`, frozen per D-02); **Founder briefing pack** — D2 review `eeea7d9d`, D3 readiness `331ca8dc`, D5 risk matrix `9e294b54`, D6 sequencing `8ee55176`.

Wave 5 fresh verifications: (1) Wave 9 API smoke ran against `b91e6c10` staging deploy — 348/6/35/389 (89.46% pass) at `reports/wave-9-20260719-173313/INDEX.md` — exceeds 2026-07-17 baseline (331/6/50/387 = 85.5%) by +17 pass / -15 skip and Phase-A-close (291/21/88 = 72.75%) by +57 pass / -15 fail / -53 skip / +16.71 pp; (2) ArchTest 51/0/10/61 fresh at `baa373aa` (+2 pass over Phase-A-close 49/0/9); (3) Doc reconciliation applied to CLAUDE.md §-1 + §0.6 + PLATFORM_MASTER_PLAN.md status header + this file. Deploy-staging.yml BLOCKED since 2026-07-18 19:53 UTC (`bd7126ab`) due to Wave 3 GAP-1 tail — logged as Wave 8.5.e continuation for Phase A.5 v2 window. Phase B FULL kick-off UNBLOCKED for scaffolding + first-slice + cross-module writes.*



*Latest (2026-07-15, Phase A CLOSED at head f3033074, Sprint Days 7-14 executed 4 days early)* — **COMMITTED TO DEVELOP** as final Phase A close-out. **Consult #27** filed (`docs/architect-consults/2026-07-14-consult-27-phase-a-close.md` per Q1-Q5 rulings): Q1 ordered 6-item close-out checklist; Q2 CI unit-test-run cited (24th deploy Wave 8.5.e known FAIL on `LankaConnect.Application.Tests` compile, workflow narrowed to `LankaConnect.API.csproj` per Consult #18); Q3 21-residual Ruling (B) accept as Wave 8.5 debt (+ 30-min PhotoAlbums body-capture confirmed same Wave 8.5.f dispatch-gap family as RSVP); Q4 `web` build via `deploy-ui-staging.yml` run `29384577093` on `f3033074` SUCCESS; Q5 Phase B scaffolding UNBLOCKED with named cross-write gate on Wave 8.5.f + 8.5.h. New artefacts: `docs/sprint/PHASE_A_CLOSED.md` (evidence + founder sign-off block) + `docs/sprint/day-14-retro.md` (what worked / what surprised / carry-forward). Sprint bible §Day 14 flipped to `DELIVERED`. Git tag `phase-a-close` on `develop@f3033074` (annotated with evidence index). Sprint totals: 24 successful staging deploys + 17 architect consults filed; Wave 9 smoke moved 43.33% → 72.75% (+29.42pp); ArchTest 49/0/9 (zero CI-blocking fails, all 9 skips carry Wave 8.5 debt refs); migration drift zero. Phase A backend structural refactor DONE per §Scope Ratified.*

*Earlier (2026-07-14, Sprint Days 7-10 chain — 24 deploys, Wave 9 43.33% → ~72%+, 2 legacy csproj deleted, Consult #25 executed, Phase A.5 Wave 8.5 debt scoped)* — **COMMITTED TO DEVELOP** across 24 sequential deploys 2026-07-11 → 2026-07-14. Sprint Day 7 execution stretched across 3 calendar days (07-12 → 07-14) instead of single-day plan due to cascading discovery of cross-module boundary bugs surfaced by Consult #20 AppDbContext sweep. **Sprint bible §Day 7 target 100+/261 pass — cleared by 141 on 20th deploy (285/27/88).** Sprint bible §Day 8 target 150+/261 — CLEARED as part of Day 7 chain. Sprint bible §Day 9 baseline 182/0/79 — PARTIALLY MET (71.25%+ pass rate, absolute pass count 285+ exceeds 182 target but on expanded 400-test suite; residual 19-27 fails documented with Phase A.5 fix path). Sprint bible §Day 10 gate "5 legacy csproj gone" — PARTIAL: 2/5 delivered (Domain, Shared), remaining 3 scoped to Phase A.5 Wave 8.5 as documented deferred debt due to Infrastructure's 566-file scope (506 EF migrations) and Application's 5-file relocation requirement. **Consult #25 filed** (`docs/architect-consults/2026-07-13-consult-25-day7-attack-order.md`): Q1 attack order approved (a→d→c→b); Q2 UoW Option B ratified (direct SaveChangesAsync blanket for single-context handlers, per-module SaveChangesInterceptor Day 8 prereq); Q3 AdminUsers 7 diagnosis revised (cascade of Q1(d) AutoMapper fix, not separate permissions issue); Q4 Day 8 folded into Day 7 chain; Q5 debt-flag sequencing; Q6 Rule 5b blanket for direct-SaveChanges pattern. **Consult #26 pending — DEFERRED to Phase A.5 Wave 8.5.j**: Events list-endpoint OwnsOne(TicketPrice).ToJson("ticket_price") MaterializeJsonEntity fails on legacy JSON shape drift; cascades to 6 downstream money-flow fails; options data-migration OR ToJson→scalar-column refactor OR custom shape-tolerant reader. **Phase A.5 Wave 8.5 debt catalog** added to docs/PHASE_A_5_PLAN.md: 12 items 8.5.a–8.5.l covering ~2-3 weeks of Wave 8.5 work — Application 5-file relocation, Infrastructure 566-file deletion + 506-migration relocation, API→Hosts/Host.AllInOne rename, LegacyPromotions folder split, test-project full-solution build restore, per-module SaveChangesInterceptor, ~95 handler IUnitOfWork migration, Wave 6.5.a shared-connection fix, metro-area cross-module write via IMetroAreaRepository, Events ToJson data-drift, Businesses controller product decision, PhotoAlbums body diagnostic. **Deploy trail SHA range**: 22cb1170..c016a622 on develop. Highlights: 14th deploy `169ad6a0` restored `[ApiController]/[Route]/[Produces]` on 4 local BaseController copies (4C.d.vi extraction regression that silently 404'd all module controllers); 15th `96a2d618` UserRepository shadow-nav guard (Wave 9 135→241 pass); 17th `494bcb8a` CreateEvent direct SaveChangesAsync split-brain fix (Wave 9 241→263 pass); 18th–20th `55ee3174/d3216796/9117e430` Consult #25 attack a+d+c (Wave 9 263→285 pass); 21st–23rd `8bcb1148/ddd1d2a8/c20a39de` RegisterUserHandler split-brain fix (POST /api/Auth/register 500→201, unblocks 7 AdminUsers + 1 Auth); 24th `c016a622` Day 10 legacy csproj deletion (Domain + Shared removed, 14 module csproj stripped of dead refs).*

*Earlier (2026-06-04, Phase A W4.1.2 attempts — BLOCKED on Communications cross-domain coupling) — **NOT COMMITTED**; learning captured in playbook + master TODO. Two W4.1.2 attempts surfaced a fundamental issue the architect-paired W3 review didn't anticipate: the Communications subdomain has cross-cutting **cultural types** (`CulturalContext` + ~15 cultural ValueObjects + ~20 cultural Services) that live structurally inside `LankaConnect.Domain/Communications/ValueObjects/` and `Services/` but are referenced by 100+ files OUTSIDE Communications (Events.Event, Common.Monitoring, Common.Security, Shared.BackupTypes, Billing). Attempt 1 (full 99-file move) hit 24 compile errors in legacy `LankaConnect.Domain` files that reference `CulturalContext`. Attempt 2 (partial 74-file move keeping cultural VOs in legacy) hit 100 compile errors at the ValueObject layer because the legacy ValueObjects depend on moved Entities/Enums — pushing the cycle one layer deeper. Conclusion: **partial moves don't help — they create cycle-trapped graphs**. Both attempts stashed (`stash@{0}` and `stash@{1}` on develop). **The fix is upstream**: pull the architect's pre-W12 BuildingBlocks elevation pass forward as a dedicated untangling workstream. Elevate the cross-cutting cultural types to `BuildingBlocks.Domain.Cultural/` (or similar). Then retry W4.1.2 cleanly. The W3.9 [MODULE_EXTRACTION_PLAYBOOK.md](docs/architecture/MODULE_EXTRACTION_PLAYBOOK.md) gains a new "When a domain isn't ready: the untangling-first pattern" section with a diagnostic command (per-type cross-reference count) so future module extractions catch this BEFORE attempting the move. **W4.2 (Media) likely has the same shape** — PhotoAlbum/AlbumPhoto are nested inside Events; architect's W4.2 plan already required a destructive `EventId → OwnerEntityId` schema reshape. Same untangling principle applies. **No regressions** — all rollbacks clean; sln still builds green; ArchTest 17/17; 303 Phase A tests still GREEN at `d353320b`.*

*Earlier (2026-06-04, Phase A W3.6 + W3.7 — NotificationsController moves into the module + Refactor.Notifications.UseNewModule flag registered)

*Earlier (2026-06-04, Phase A W4.1.1 + W4.2.1 — Communications + Media module skeletons) — **COMMITTED TO DEVELOP**. Per W3.9 playbook §"Step 1 — Module skeleton", 18 csprojs landed: 5 src + 4 test under `src/Modules/Communications/` and `src/Modules/Media/`. Each source project has an `AssemblyMarker.cs` placeholder anchoring NetArchTest until real types arrive. ProjectReferences enforce Clean Architecture inward (Domain→BuildingBlocks.Domain only; Contracts→BuildingBlocks.Contracts only; Application→Domain+Contracts+BuildingBlocks.Application; Infrastructure→Application+Domain+Contracts+BuildingBlocks.Infrastructure; Api→all four+BuildingBlocks.Web). All 18 added to `LankaConnect.sln`. **8 new NetArchTest rules** (4 per module) added to LayeringRules.cs enforcing that neither module references LankaConnect.{Application,Infrastructure,API,Shared} or any OTHER `Modules.*` module — including a fresh `Modules.Notifications.*` exclusion now that Notifications exists. **ArchTest 17/17 GREEN** (was 9/9: +4 Communications + 4 Media module-boundary rules). All builds clean (Communications.Api + Media.Api + ArchTest assembly; 0 warnings, 0 errors). **Scope to be moved in subsequent W4 substeps** (per playbook): Communications = 99 domain files + 100 application files + 54 infra files (non-migrations) + 59 typed email-param classes from `LankaConnect.Shared/Email/` + 9 controllers (Email/EmailGroups/EmailMetrics/AdminEmailTemplates/Newsletter/Newsletters/WhatsApp/WhatsAppAdmin/WhatsAppWebhook); Media = 2 domain entities (PhotoAlbum, AlbumPhoto) + 2 domain events + 1 enum + 13 application files + 1 controller (PhotoAlbumsController). **Communications is the BIGGEST extraction by file count of any Phase A module** — substeps W4.1.2 (Domain move) and W4.1.4 (App + Infra move) each land in dedicated sessions to keep diffs reviewable. Media is roughly Notifications-sized.*

*Earlier (2026-06-04, Phase A W3.8 staging deploy + flag flip verified + W3.9 playbook written + W3.10 close-out) — **COMMITTED TO DEVELOP**. **W3.8 code-verifiable acceptance MET**: (1) push `2673a319` deployed via run `26930080528`; first flag-OFF smoke caught `MediatR.IRequestHandler<>` not registered (handler assembly invisible to MediatR's `LankaConnect.Application.dll`-only scan); fix `ea0b0313` had `NotificationsModule.AddNotificationsModule` call `AddMediatR(cfg => cfg.RegisterServicesFromAssemblies(typeof(GetUnreadNotificationsQueryHandler).Assembly))`. Redeploy `26931041557`; re-smoke `GET /api/Notifications/unread` → HTTP 200 `[]`. Container log confirms `LankaConnect.Modules.Notifications.Api.Controllers.NotificationsController: ... (UseNewModule=False)` — flag pipeline observed end-to-end. Flag flipped ON via `az containerapp update --set-env-vars "FeatureManagement__Refactor.Notifications.UseNewModule=true"`; re-smoke HTTP 200 `[]`; container log shows `UseNewModule=True`. (2) **API baseline regression GREEN**: `bash tests/api-baseline/run-baseline-regression.sh` reports 312 paths / 403 schemas unchanged from W2.8 capture → **module extraction did not drift the public API surface**. (3) Flag reverted to OFF (appsettings default). 7-day calendar soak proceeds wall-clock. **W3.9 [MODULE_EXTRACTION_PLAYBOOK.md](docs/architecture/MODULE_EXTRACTION_PLAYBOOK.md)** authored: 10-step extraction sequence (matches W3.1-W3.10 substeps) with file-and-code-level instructions, transitional-debt callouts, microservice-readiness checklist. Module map names every Phase A shared-infra module (Notifications/Communications/Media/Forms/Payments/Events/Identity) + every known Phase 2+ product module — LankaEvents (live), LankaSeyla/LankaMart/LankaNivasa (Commerce per ADR-002), LankaBusiness (Directory), **LankaHomes + LankaTemples** per architect's pair-review nit. Pitfalls table captures 10 real issues hit during W3. **W3.10 close-out**: this entry + master TODO substeps marked `[x]` with verification evidence. **Phase A test totals after W3**: BuildingBlocks suites unchanged from W2 close (290+ GREEN); ArchTest 9/9 GREEN (4 BuildingBlocks + 4 Notifications module-boundary + 1 anchor); API baseline regression GREEN (zero drift). **Up next**: W4 — Communications + Media modules (parallelizable per master TODO). Communications absorbs 41 typed-email-param classes into its Contracts. Both follow the playbook authored in W3.9.*

*Earlier (2026-06-04, Phase A W3.6 + W3.7 — NotificationsController moves into the module + Refactor.Notifications.UseNewModule flag registered)

*Earlier (2026-06-04, Phase A W3.6 + W3.7 — NotificationsController moves into the module + Refactor.Notifications.UseNewModule flag registered) — **COMMITTED TO DEVELOP**. **W3.6** moves `NotificationsController.cs` from `src/LankaConnect.API/Controllers/` to `src/Modules/Notifications/Notifications.Api/Controllers/`; namespace flips to `LankaConnect.Modules.Notifications.Api.Controllers`. Controller inherits `ControllerBase` directly (NOT the legacy `BaseController<T>`) because inheriting it would close a hard cycle `LankaConnect.API → Notifications.Api → LankaConnect.API`. The handful of helpers (`HandleResult`, `HandleResultUnit`, `BuildProblem`, `TryGetUserId`) are inlined — ~30 LOC at the bottom of the controller class. Future work elevates a reusable `ModuleControllerBase` into `BuildingBlocks.Web`. **Feature-flag observation wired into every endpoint**: controller injects `IFeatureManager` and each endpoint calls `IsEnabledAsync("Refactor.Notifications.UseNewModule")` and emits a log line `UseNewModule={true|false}` for end-to-end flag-pipeline visibility during the W3.8 soak. The flag has NO BEHAVIORAL EFFECT during W3 (new and legacy code paths produce identical SQL against the same physical `notifications.notifications` table) — but the observability proves the flag pipeline is alive end-to-end and gives a future hook if W3.x dual paths emerge. **Host wiring** in `LankaConnect.API/Program.cs`: `.AddApplicationPart(typeof(LankaConnect.Modules.Notifications.Api.Controllers.NotificationsController).Assembly)` so MVC discovers the controller from the referenced module assembly. **Transitional edges added** (documented in csproj): `Notifications.Api` references `Microsoft.FeatureManagement.AspNetCore` package + `LankaConnect.Domain` (for `Result<T>` — cut alongside the BuildingBlocks elevation pass). **W3.7** registers the flag: (a) `Refactor.Notifications.UseNewModule: false` added to `src/LankaConnect.API/appsettings.json` `FeatureManagement` section; (b) registry row added to [docs/feature-flags.md](docs/feature-flags.md) — category Refactor, sunset 2026-07-01 (W7), default `false`, description states "When `false` (default), `INotificationRepository` resolves to the legacy implementation that injects `AppDbContext`. When `true`, resolves to the module implementation that injects `NotificationsDbContext`. Sunsets W7 alongside legacy `NotificationRepository` removal." **Pragmatic note documented in master TODO**: the dual-DbContext factory routing is DEFERRED — the two paths are functionally identical during W3 and the flag's purpose here is procedural + a future hook. **Real dual-path flags land starting W4** where modules genuinely diverge. **Verification**: full sln build green (0 errors, 8 pre-existing NU190x vuln warnings unchanged); ArchTest 9/9 GREEN. **W3.8 next**: push triggers `deploy-staging.yml`; smoke `/api/Notifications/unread` with flag OFF (default — expected response unchanged); flip flag ON in staging via appsettings override; smoke again; 7-day soak monitoring App Insights for error rate + p50/p95/p99 deltas vs the W2.6b baseline.*

*Earlier (2026-06-03, Phase A W3.5b —

*Earlier (2026-06-03, Phase A W3.5b — Notifications operational tables migration) — **COMMITTED TO DEVELOP**. Per master TODO W3.5 acceptance, the per-schema operational tables landed via a SECOND migration `Add_NotificationsOperationalTables` so the baseline (W3.5a) stays a true history-only marker. NEW shared types in `src/BuildingBlocks/BuildingBlocks.Infrastructure/`: (1) `Idempotency/IdempotencyKey.cs` — entity (Key Guid PK + SerializedResponse jsonb + RecordedAt + ExpiresAt) with `Create(...)` static factory enforcing non-empty key + non-empty response + ExpiresAt strictly after RecordedAt; (2) `Idempotency/IdempotencyKeyConfiguration.cs` + (3) `Outbox/OutboxMessageConfiguration.cs` + (4) `Outbox/DeadLetterMessageConfiguration.cs` — reusable EF configurations every module DbContext applies; outbox partial index `WHERE ProcessedAt IS NULL` keeps the processor hot path fast as historical processed rows accumulate. **NotificationsDbContext** now exposes three new DbSets (`Outbox`, `OutboxDeadLetter`, `IdempotencyKeys`) and applies all three configs in `OnModelCreating`. **Generated migration**: `Migrations/20260604035116_Add_NotificationsOperationalTables.cs` + companion `.Designer.cs` (verified `[Migration("...")]` at line 15 per [MEMORY.md hand-create rule](C:/Users/Niroshana/.claude/projects/c--Work-LankaConnect/memory/MEMORY.md)). Creates 3 tables in `notifications` schema with PKs + 4 indexes (TTL sweep on idempotency_keys.ExpiresAt; partial pending-row index on outbox.OccurredAt; original-id + dead-lettered-at indexes on outbox_dead_letter). Down() drops all three. `NotificationsDbContextModelSnapshot.cs` updated. **Verification**: full sln build green (0 errors, 8 pre-existing NU190x vuln warnings unchanged); ArchTest 9/9 GREEN. **W3.5c next** (deferred to a real staging cycle): `dotnet ef database update --context NotificationsDbContext --project src/Modules/Notifications/Notifications.Infrastructure --startup-project src/LankaConnect.API` against a staging clone, then `migra postgresql://staging postgresql://production` to verify zero structural drift. **Then W3.6**: NotificationsController feature-flag wiring (route on `Refactor.Notifications.UseNewModule` flag value); the controller stays in legacy `LankaConnect.API/Controllers/` for this round (full move to `Notifications.Api/Controllers/` is a follow-up after the flag ramps to 100%).*
